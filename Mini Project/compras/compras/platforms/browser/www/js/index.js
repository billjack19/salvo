...
function prodScan() {
	...
	cordova.plugins.barcodeScanner.scan(
		function (result) {
			alert(result.text);
			buscarSetarProduto(result.text, false);
		}, 
		function (error) {
...
function buscarSetarProduto(cod, boolModal){
	console.log("buscarSetarProduto");
	$("#modalCodFunc").modal("hide");
	cod = cod.match(/[0-9]/g).join('');
	...
	if(cod.length >= 13){
		$.ajax({
			...
			error: function(){
				alert("Falha ao tentar pesquisar Funcionario");
			}
		}).done( function(data){
			console.log(data);
			try{
...
function salvarProduto(onde){
	...
	} else if( onde == "c" && ( valorUnitario == "" || isNaN(valorUnitario) ) ){
