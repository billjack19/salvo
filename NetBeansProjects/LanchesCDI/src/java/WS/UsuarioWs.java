...
public class UsuarioWs {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Path("/logar")
    public Response inserirJogador(String content) throws SQLException{
        Gson g = new Gson();
        TabUsuarioDao _TabUsuarioDao = new TabUsuarioDao();
        TabUsuarios _Usuario = (TabUsuarios) g.fromJson(content, TabUsuarios.class);

        List<TabUsuarios> TabUsuarios = _TabUsuarioDao.autenticaJogador(_Usuario.getIdentificador(), _Usuario.getSenha());
        return Response.ok()
                .entity(g.toJson(TabUsuarios))
                .header("Access-Control-Allow-Origin", "*")
                .header("Access-Control-Allow-Methods", "POST, GET, PUT, UPDATE, OPTIONS")
