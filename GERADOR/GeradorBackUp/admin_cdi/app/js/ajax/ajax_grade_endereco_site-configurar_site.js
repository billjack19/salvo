...
function buscar_endereco_site(){
	...
	for (var i = grades.length - 1; i >= 0; i--) {
		if ($(grades[i]).data("p") == "configurar_site" && $(grades[i]).data("g") == "endereco_site") {
	...
	$.ajax({
		...
		if (subvetor[1] == undefined) {
			...
			for (var i = 0; i < vetor.length; i++) {
				...
				tabelaViewBody +=				"<a href='principal.php#!editar_grade_endereco_site-configurar_site' style='color: #f0ad4e;' data-id='"+id_endereco_site+"' onclick='editar(this);' title='Editar'"+accesskeyEditar+" "+desabilitar+">";
				tabelaViewBody +=				 	"<b><i class=\"fa fa-pencil\" aria-hidden=\"true\"></i></b>";
				tabelaViewBody += 				"</a>";
				tabelaViewBody +=			"</td>";
				tabelaViewBody += 			"<td align='center'>";
				tabelaViewBody += 				"<div  id='ativo_"+id_endereco_site+"'>";
				tabelaViewBody += 				"<a href='#!grade_endereco_site-configurar_site' style='color: "+cor_ativo+"' data-id='"+id_endereco_site+"' onclick=\"excluir(this , 'endereco_site', "+bool_ativo_endereco_site+", 'grade_endereco_site-configurar_site')\" title='Excluir'>";
		...
		var botaoBoltarGrade = verificaGrade('configurar_site', 'endereco_site', 'Configurar Site');
