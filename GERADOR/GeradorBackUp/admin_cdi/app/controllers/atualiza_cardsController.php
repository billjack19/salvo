...
<?php 
	...
	if ($gravar) {
		...
		$entidadeCards->set($_POST['descricao_item_cards'], 'descricao_item_cards');
		$entidadeCards->set($_POST['imagem_cards'], 'imagem_cards');

		$sequencia_cards = $_POST['sequencia_cards'] == '' ? 0 : $_POST['sequencia_cards'];
		$entidadeCards->set($sequencia_cards, 'sequencia_cards');
		...
		$retorno = $cardsDAO->atualizaCards($entidadeCards, $_POST['id_cards']);
		echo $retorno;
	}
	else echo $saida;
?>
