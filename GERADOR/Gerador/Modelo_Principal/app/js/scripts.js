...
* facilitar no desenvolvimento contém as chamas funções "genêricas"
/************************************************************************************************************************/
...
function verificaAcess(area, especificacao){
	var result = "";
	$.ajax({
		...
		console.log(data);
		data = data.split("-");
		if (data[0] == 0 || data.indexOf("L") == -1) window.location.assign("principal.php#!acesso_bloqueado");
		else {
			if (data.indexOf("I") == -1 && especificacao == "I") {
				window.location.assign("principal.php#!acesso_bloqueado");
			} else if(data.indexOf("I") == -1){
				var adiconar = document.getElementsByName("adiconar");
				for (var i = 0; i < adiconar.length; i++) adiconar[i].style.display = "none";
			}

			if (data.indexOf("A") == -1 && especificacao == "A") {
				window.location.assign("principal.php#!acesso_bloqueado");
			} else if (data.indexOf("A") == -1){
				var btn_editar = document.getElementsByName("btn_editar");
				for (var i = 0; i < btn_editar.length; i++) btn_editar[i].style.display = "none";
			}
		}
		/* Old verificação */
...
function setarValorEstrangeiroLista(id, tabelaEstrangeira, complemento){
	...
	var param = JSON.parse('{ "' + colunaParam + '":true, "id":' + idTabelaEstrangeira + ', "oldObject": true }');
	console.log(param);
