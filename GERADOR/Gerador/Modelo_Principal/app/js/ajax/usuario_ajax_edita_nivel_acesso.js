...
function formatarAreasAcesso(area_nivel_acesso){
	...
	for (i in area_nivel_acesso){
		...
		ctrlClass = "danger"; // area_nivel_acesso[i].list == 1 ? "success" : "danger";
		ctrlIcon = "times"; // area_nivel_acesso[i].list == 1 ? "check" : "times";
		ctrlDisabled = area_nivel_acesso[i].list == 1 ? "" : " disabled";

		tabelaColunas +=		"<div id='btn_area_l_"+area_nivel_acesso[i].area+"'>";
		tabelaColunas += 		"<button style='margin-bottom: 10px' class='btn btn-"+ctrlClass+" btn-block' onclick='definirAreaAcessoLIU(\""+area_nivel_acesso[i].area+"\", \"l\", 1)'"+ctrlDisabled+">";
		...
		tabelaColunas += 		"<button style='margin-bottom: 10px' class='btn btn-"+ctrlClass+" btn-block' onclick='definirAreaAcessoLIU(\""+area_nivel_acesso[i].area+"\", \"i\", 1)'"+ctrlDisabled+">";
		...
		tabelaColunas += 		"<button style='margin-bottom: 10px' class='btn btn-"+ctrlClass+" btn-block' onclick='definirAreaAcessoLIU(\""+area_nivel_acesso[i].area+"\", \"u\", 1)'"+ctrlDisabled+">";
	...
	selecionarAreasDeAcesso();
}


function selecionarAreasDeAcesso(){
	$.ajax({
		url:'app/controllers/funcoes_nivel_acessoController.php',
		type: "POST",
		dataType: "text",
		...
		try{
			...
			for (var i = 0; i < data.length; i++) {
				if(data[i].bool_list_area_nivel_acesso   == 1) 	definirAreaAcessoLIU(data[i].area_area_nivel_acesso, "l", 1);
				if(data[i].bool_insert_area_nivel_acesso == 1) 	definirAreaAcessoLIU(data[i].area_area_nivel_acesso, "i", 1);
				if(data[i].bool_update_area_nivel_acesso == 1) 	definirAreaAcessoLIU(data[i].area_area_nivel_acesso, "u", 1);
			}
		}catch(error){
			console.log("Falha ao tentar converter objeto em JSON: " + error);
		}
	});
