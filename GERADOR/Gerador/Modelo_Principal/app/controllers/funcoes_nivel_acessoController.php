...
if (!empty($_POST['pequisa_nivel_acesso_id'])) {
	...
	$resultado = "[";
	$ctrlVirgula = "";
	$sql = "	SELECT area_nivel_acesso.*, nivel_acesso.descricao_nivel_acesso
				FROM area_nivel_acesso area_nivel_acesso
	...
	$verifica = $pdo->query($sql);
	foreach ($verifica as $dados) {
		if (!empty($_POST['oldObject'])) {
			$resultado = 	$dados['demostrativo_area_nivel_acesso']."{,}".
	...
	$resultado .= !empty($_POST['oldObject']) ? "" : "]";
	echo $resultado;
} 
	...
	*/
	...
	if ($gravar) {
		$sql = !empty($_POST['id'])
				 ?
				"UPDATE nivel_acesso
				 SET descricao_nivel_acesso = '$descricao_nivel_acesso', area_nivel_acesso = '', usuario_id = $usuario_id
				 WHERE  d_nivel_acesso = ".$_POST['id']
				 :
				"INSERT INTO nivel_acesso (descricao_nivel_acesso, area_nivel_acesso, usuario_id)
...
if (!empty($_POST['adicionar_area_nivel_acesso'])) {
	...
	$sql = "DELETE FROM area_nivel_acesso 
			WHERE nivel_acesso_id = $nivel_acesso_id 
			AND area_area_nivel_acesso = '$area_area_nivel_acesso'";
	$stmt = $pdo->prepare($sql);
	$stmt->execute();
...
if (!empty($_POST['areasDeAcesso'])) {
	$id = $_POST['id'];

	$area_acesso = new Area_acesso();
	$array_Area_acesso = array();

	$resultado = ""; $cont = 0;

	$sql = "SELECT * FROM area_nivel_acesso WHERE nivel_acesso_id = $id";
	$verifica = $pdo->query($sql);
	foreach ($verifica as $dados) {
		if ($cont !=  0) 
		$resultado .=    "[]";
		$resultado .=
				$dados['id_area_nivel_acesso']."{,}".
				$dados['area_area_nivel_acesso']."{,}".
				$dados['demostrativo_area_nivel_acesso']."{,}".
				$dados['bool_list_area_nivel_acesso']."{,}".
				$dados['bool_insert_area_nivel_acesso']."{,}".
				$dados['bool_update_area_nivel_acesso']."{,}".
				$dados['data_atualizacao_area_nivel_acesso']."{,}".
				$dados['usuario_id']."{,}".
				$dados['bool_ativo_area_nivel_acesso'];
		$cont++;

		$area_acesso = new Area_acesso();

		$area_acesso->set($dados['id_area_nivel_acesso'], 				"id_area_nivel_acesso"				);
		$area_acesso->set($dados['area_area_nivel_acesso'], 			"area_area_nivel_acesso"			);
		$area_acesso->set($dados['demostrativo_area_nivel_acesso'], 	"demostrativo_area_nivel_acesso"	);
		$area_acesso->set($dados['bool_list_area_nivel_acesso'], 		"bool_list_area_nivel_acesso"		);
		$area_acesso->set($dados['bool_insert_area_nivel_acesso'], 		"bool_insert_area_nivel_acesso"		);
		$area_acesso->set($dados['bool_update_area_nivel_acesso'], 		"bool_update_area_nivel_acesso"		);
		$area_acesso->set($dados['data_atualizacao_area_nivel_acesso'], "data_atualizacao_console"			);
		$area_acesso->set($dados['usuario_id'], 						"usuario_id"						);
		$area_acesso->set($dados['bool_ativo_area_nivel_acesso'], 		"bool_ativo_area_nivel_acesso"		);

		array_push($array_Area_acesso, $area_acesso);
	}
	echo !empty($_POST['new_object']) ? toJson($array_Area_acesso) : $resultado;
}
