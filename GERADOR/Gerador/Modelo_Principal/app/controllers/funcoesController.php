...
if (!empty($_POST['acesso'])) {
	...
	$sql = "SELECT 
			...
			WHERE nivel_acesso_id = (SELECT nivel_acesso_id FROM usuario WHERE id_usuario = $idUsusario)
			AND area_area_nivel_acesso = '$area'";

	$verifica = $pdo->query($sql);
	foreach ($verifica as $dados){
		$acesso = 	($dados['bool_list_area_nivel_acesso']   == 1 ?  "L" : "") .
					($dados['bool_insert_area_nivel_acesso'] == 1 ? "-I" : "") .
					($dados['bool_update_area_nivel_acesso'] == 1 ? "-A" : "");
	}	
	echo $acesso;
}
