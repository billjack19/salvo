...
$classeNomeViewCadastro = "adicionar_".$nomeTabela;
...
foreach ($verifica as $dados) {
	if ($dados[3] == "PRI") {
		...
		if ($nomeCampoOriginal == TAB_LOGIN."_id") {
			...
			$inputDefinido = "<div id=\"".$tabelaEstrangeira."Div\">
		<input type=\"text\" value=\"Aguardando seleção...\" class=\"form-control\" disabled>
	</div>";
			if (!in_array($nomeCampoOriginal, $colunasBuscasRetroativas)) {
				$scrpitsCombo .= "
<script type=\"text/javascript\" src=\"app/js/combo/".$tabelaEstrangeira."Combo.js\"></script>";
			}
			else {
				$scrpitsCombo .= "
<script type=\"text/javascript\" src=\"app/js/combo/".$tabelaEstrangeira."ComboGrade.js\"></script>";	
		...
		else if ($verificaCampoBool == "senha") {
			$inputDefinido = "<input class=\"form-control\" type='password' id=\"$nomeCampoOriginal\" $accesskeyInicial>";
		}


		// verifica camopos que Começam com nome de tipo de arquivo
		else if ($verificaCampoBool == "imagem") {
			$inputDefinido = "<button onclick=\"chamarModalImagem('$nomeCampoOriginal', '$classeNomeViewCadastro', 'img')\" class=\"btn\" $accesskeyInicial>
		Selecionar Imagem
	</button>
	<input type=\"text\" class=\"form-control\" id=\"$nomeCampoOriginal\" disabled>";
			...
			$inputDefinido = "<button onclick=\"chamarModalImagem('$nomeCampoOriginal', '$nomeTabela', 'video')\" class=\"btn\" $accesskeyInicial>
		Selecionar Vídeo
	</button>
	<input type=\"text\" class=\"form-control\" id=\"$nomeCampoOriginal\" disabled>";
		}
			...
			$inputDefinido = "<button onclick=\"chamarModalImagem('$nomeCampoOriginal', '$nomeTabela', 'audio')\" class=\"btn\" $accesskeyInicial>
		Selecionar Áudio
	</button>
	<input type=\"text\" class=\"form-control\" id=\"$nomeCampoOriginal\" disabled>";
		}
			...
			$inputDefinido = "<button onclick=\"chamarModalImagem('$nomeCampoOriginal', '$nomeTabela', 'text')\" class=\"btn\" $accesskeyInicial>
	...
	<input type=\"text\" class=\"form-control\" id=\"$nomeCampoOriginal\" disabled>";
		...
		// verifica se é texto
			...
			$inputDefinido = "<input type=\"text\" maxlength=\"$tamanhoCampo\" class=\"form-control\" id=\"$nomeCampoOriginal\"$disabilitar $accesskeyInicial>";
		}

		// verifica se é muito texto kkk
		else if (
...
$contrudoViewCadastro = "

<h1>Adicionar: $nomeTableForm</h1>

<div class=\"text-right\">
	<a href=\"principal.php#!$nomeTable\" class=\"btn btn-primary\" accesskey=\"v\">
...
<div class=\"row\">$campos
	<div class=\"col-md-12 text-center\">
		<br>
		$inputHidden
		<button class=\"btn btn-success\" onclick='gravarRegistro()' accesskey=\"s\">
...
<script type=\"text/javascript\">
	verificaAcess('".$nomeTable."', 'I');

	$scriptAdicionaisComboGrade
</script>

<script type=\"text/javascript\" src=\"app/js/ajax/ajax_adicionar_$nomeTable.js\"></script>
$scrpitsCombo
";
