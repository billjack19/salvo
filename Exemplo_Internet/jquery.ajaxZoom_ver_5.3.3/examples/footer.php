...
if ($isAjaxZoomCom && file_exists(dirname(__FILE__).'/comments.php')) {

	if ($isAjaxZoomCom) {
		$currentExample = basename($_SERVER['PHP_SELF']);
		$currentIndex = intval(str_replace('example', '', $currentExample));

		if ($currentIndex.'' === str_replace('example', '', str_replace('.php', '', $currentExample))) {
			$example_comment = $currentIndex;
...
if ($isAjaxZoomCom) {
	echo '<iframe width="100%" height="230" frameborder="0" src="nav_thumbs.php?'.$navThimbsQueryString.'" allowfullscreen></iframe>';
} else {
	?>
