...
;(function($) {
	...
	$.axZmEb = function(opt) {
		...
		var def = {
			...
			lnk: null, // link, instead of description
			fnc: null, // function instead of description
		...
		var onZoomSpinPanOnce = function() {
			...
			if (typeof o.zoomSpinPanRemove == 'string') {
				if (o.zoomSpinPanRemove.indexOf('#') === 0 || o.zoomSpinPanRemove.indexOf('.') === 0) {
					try {
			...
			.bind('click', function(e) {
				...
				if (_tButton.css('zIndex') < 10) {
					...
					if (o.parO == 'body' && o.par != 'body') {
						...
						if (top) {
							_tButton.css('top', ofs.top - scrT);
						}

						if (right) {
							_tButton.css('right', $(window).width() - ofs.left + scrL - _tButton.outerWidth());
						}

						if (bottom) {
							_tButton.css('bottom', $(window).height() - ofs.top + scrT - _tButton.outerHeight());
						}

						if (left) {
							_tButton.css('left', ofs.left - scrL);
						}
					}
	...
	var axZmEbAddResizeListener = function(element, fn) {
		if (attachEvent) {
			...
			if (!element.__resizeTriggers__) {
				...
				element.__resizeTriggers__.innerHTML = '<div class="axZmEb-expand-trigger"><div></div></div>' + '<div class="axZmEb-contract-trigger"></div>';
