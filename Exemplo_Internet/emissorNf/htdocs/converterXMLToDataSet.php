...
try{	
	...
	while( $spdNFeDataSets->FindDataSet("Y")->Eof() === false ){
		$faturasArray = array(
			...
			"vDesc"	 =>	$spdNFeDataSets->GetCampo("vDesc_Y05"),
			"vLiq"	 =>	$spdNFeDataSets->GetCampo("vLiq_Y06")
			);		
		$spdNFeDataSets->FindDataSet("Y")->Next();
	$faturaArray[] = $faturasArray;
	echo(json_encode($faturaArray));
	}

	//Informações das duplicatas
	$duplicatasArray = array();
	$duplicataArray = array();
	...
	$duplicataArray[] = $duplicatasArray;
	echo(json_encode($duplicataArray));
	}
