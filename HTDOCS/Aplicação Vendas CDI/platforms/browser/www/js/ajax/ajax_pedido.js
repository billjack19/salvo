function montarViewAdicionaPedido(){
	...
	$.ajax({
		...
		if (data.length == 0) {
			...
			motarSubMenu += "<h4>Consumidor: ";
			// motarSubMenu += "</td><td>";
			motarSubMenu += "<input type='checkbox' style='font-size:100px;' id='checkboxConsumidor' onclick='confereCheckConsumidor();'>";
...
function montarListaItem(lancPedidoId){
	...
	$.ajax({
		...
		for(i in data){
			...
			motarSubMenu += "<td colspan='2'><h4>Cliente</h4>";
