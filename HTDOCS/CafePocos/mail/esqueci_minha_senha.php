...
if (!empty($_POST['rf'])) {
	...
	$sql = "SELECT cliefornec_telefone.Email 
			...
			INNER JOIN cliefornec_telefone cliefornec_telefone ON cliefornec.CODIGO = cliefornec_telefone.Cliente
...
)
{
	...
	$sql = "UPDATE cliefornec 
			SET senha_site = concat(Cpf_cgc,\"\",\"14122017\")
			WHERE CPF_CGC = '$loginPost'";
	...
	$sql = "SELECT senha_site, RAZAOSOCIAL
			FROM cliefornec 
			WHERE CPF_CGC = '$loginPost'";
