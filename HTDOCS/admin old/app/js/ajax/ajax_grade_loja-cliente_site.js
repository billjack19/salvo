...
$(document).ready(function(){
	...
	for (var i = grades.length - 1; i >= 0; i--) {
		if ($(grades[i]).data("p") == "cliente_site" && $(grades[i]).data("g") == "loja") {
	...
	$.ajax({
		...
		if (subvetor[1] == undefined) {
			...
			for (var i = 0; i < vetor.length; i++) {
				...
				data_atualizacao_loja = subvetor[4];
				bool_ativo_loja = subvetor[5];
				...
				tabelaViewBody +=				"<a href='principal.php#!editar_grade_loja-cliente_site' style='color: #f0ad4e;' data-id='"+id_loja+"' onclick='editar(this);' title='Editar' "+desabilitar+">";
				...
				tabelaViewBody += 			"<td align='center'>";
				tabelaViewBody += 				"<a href='#!grade_loja-cliente_site' style='color: "+cor_ativo+"' data-id='"+id_loja+"' onclick=\"excluir(this , 'loja')\" title='Excluir'>";
		...
		var botaoBoltarGrade = verificaGrade('cliente_site', 'loja', 'Cliente Site');
