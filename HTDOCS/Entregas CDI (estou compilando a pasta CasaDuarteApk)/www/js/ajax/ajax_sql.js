...
function criarBancoDeDados(){
	bancoDeDados =  openDatabase("EntregaCDI", "1.0", "Testar SQL para logar sozinho", 200000);
...
function adicionarIp(){
	...
	if ((modalUsuarioConfig == "ADM" || modalUsuarioConfig == "Adm" || modalUsuarioConfig == "adm") && modalChaveConfig == "2017") {
		...
		if (ip == "") {
			...
			hostWebService = ip;
			hostServeImage = ip;
			urlWebService = "http://"+hostWebService+caminhoWebService;//"http://"+hostWebService+portaWebService+caminhoWebService;
			// setarIpCampoConfig();
