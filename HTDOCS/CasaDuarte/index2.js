...
var rotas_Global = []; /* Fundamental para tracar as rotas */
		...
		for(i in data){
			if (data[i].endEntrega.length > 0) {
				...
				for (var j = 0; j < data[i].endEntrega.length; j++) {
					waypts.push({
						location: data[i].endEntrega[j].endEntregaItem, 
						stopover: true
					});
				}
				...
				data[i].entregas = waypts;
				data[i].indice = i;
				data[i].corObject = arrayDeCores[indiceCor];

				rotas_Global.push(data[i]);
			}
		}
		console.log(rotas_Global);
		console.log(JSON.stringify(rotas_Global));
...
function setarRotaExclusiva(numeroDaRota){
	...
	var indece = document.getElementById("rota_"+numeroDaRota).value;
	rotasCordenadas_Global[indece].setMap(map);
