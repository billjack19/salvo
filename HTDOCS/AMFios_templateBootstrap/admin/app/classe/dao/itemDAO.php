...
class itemDAO{
	...
	function cadastraItem(Item $entidadeItem, $area){
		...
		$descricaoNotificacao .= 'Descrição Site => '.$entidadeItem->get('descricao_site_item').'<br>';
		$descricaoNotificacao .= 'Unidade Medida => '.$entidadeItem->get('unidade_medida_item').'<br>';
		$descricaoNotificacao .= 'Imagem => '.$entidadeItem->get('imagem_item').'<br>';
	...
	function atualizaItem(Item $entidadeItem, $id, $area){
		...
		foreach ($verifica as $dados){ 
			...
			if ($dados['descricao_site_item'] != $entidadeItem->get('descricao_site_item')) {
				$descricaoNotificacao .= '<b style="color: red">Descrição Site: '.$dados['descricao_site_item'].' => '.$entidadeItem->get('descricao_site_item').'</b><br>';
				$controleAteracao++;
			}
			else {
				$descricaoNotificacao .= 'Descrição Site => '.$entidadeItem->get('descricao_site_item').'<br>';
			}
			if ($dados['unidade_medida_item'] != $entidadeItem->get('unidade_medida_item')) {
				$descricaoNotificacao .= '<b style="color: red">Unidade Medida: '.$dados['unidade_medida_item'].' => '.$entidadeItem->get('unidade_medida_item').'</b><br>';
				$controleAteracao++;
			}
			else {
				$descricaoNotificacao .= 'Unidade Medida => '.$entidadeItem->get('unidade_medida_item').'<br>';
			}
			if ($dados['imagem_item'] != $entidadeItem->get('imagem_item')) {
				$descricaoNotificacao .= '<b style="color: red">Imagem: '.$dados['imagem_item'].' => '.$entidadeItem->get('imagem_item').'</b><br>';
				$controleAteracao++;
			}
			else {
				$descricaoNotificacao .= 'Imagem => '.$entidadeItem->get('imagem_item').'<br>';
