...
if (!empty($_POST['pequisa_upload_arq_filtro'])) {
	$filtro = $_POST['filtro'];
	$cont = 1;
	$sql = "	SELECT upload_arq.* 
				FROM upload_arq upload_arq  
				WHERE descricao_upload_arq LIKE '%$filtro%'
				OR tipo_upload_arq LIKE '%$filtro%'
				OR data_atualizacaoupload_arq LIKE '%$filtro%'
				OR bool_ativo_upload_arq LIKE '%$filtro%'
			";
	$verifica = $pdo->query($sql);
	foreach ($verifica as $dados) {
		if ($cont ==  1) {
...
if (!empty($_POST['pequisa_upload_arq_filtro_grade'])) {
	$filtro = $_POST['filtro'];
	$coluna = $_POST['tabela']."_id";
	$id = $_POST['id'];
	...
	$sql = "	SELECT upload_arq.* 
				...
				WHERE $coluna = $id 
				AND (
					   descricao_upload_arq LIKE '%$filtro%'
					OR tipo_upload_arq LIKE '%$filtro%'
					OR data_atualizacaoupload_arq LIKE '%$filtro%'
					OR bool_ativo_upload_arq LIKE '%$filtro%'
				)
