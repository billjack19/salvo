		...
		if (i != paradas.length - 1) {
			...
			arrayIndiceMarcador[arrayIndiceMarcador.length - 1].push(
				adicionarMarcador(
					...
					objetoEntrega[indiceRealEnterga]/*.codigo */
				)
			);
			...
			if (setarCaminhaoMapa && caminhaos[numeroIndeceDefinitivo].setarMapa) {
				...
				$.ajax({
					...
					caminhaos[numeroIndeceDefinitivo].caminhao.addListener('click', function() {
						...
						setarRegistroCaminhao(this.title, this.motorista);
					});
...
function atualizarPacotes(arrayCodigos){
	setInterval( function(){
		for (var i = 0; i < arrayCodigos.length; i++) {
			$.ajax({
				type: 'GET',
				url: urlWebService+"ViagemSimplesItemWs/pesquisa/"+arrayCodigos[i]+parametrosWebService,
				contentType: "application/json",
				jsonpCallback: "localJsonpCallback"
			}).done( function(data){
				var indiceImagemReference = "";
				...
				if (data.bool_entrega == 1) {
					for (var j = 0; j < MarcadoreEntregas.length; j++) {
						if (data.codigo == MarcadoreEntregas[j].Marker.codigo) {
							indiceImagemReference = document.getElementById("marcador_indice_"+data.codigo).value;
							imagemSetada = "img/"+corConfirmacao+"/pacote_"+corConfirmacao+"_"+indiceImagemReference+".png";
							document.getElementById("imagem_pacote_"+data.codigo).src = imagemSetada;
