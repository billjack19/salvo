...
class cardsDAO{
	...
	function cadastraCards(Cards $entidadeCards){
		try{
			...
			$stmt = $this->pdo->prepare("INSERT INTO cards (titulo_cards, descricao_cards, imagem_cards, bool_ativo_cards) VALUES (:titulo_cards, :descricao_cards, :imagem_cards, :bool_ativo_cards);"
			);
	...
	function atualizaCards(Cards $entidadeCards, $id){
		try{
			...
			$stmt = $this->pdo->prepare("UPDATE cards SET titulo_cards = :titulo_cards, descricao_cards = :descricao_cards, imagem_cards = :imagem_cards, bool_ativo_cards = :bool_ativo_cards WHERE id_cards = ".$id.";"
			);
			return $stmt->execute($param);
		}catch(PDOException $ex){
