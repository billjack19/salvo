...
class lojaDAO{
	...
	function cadastraLoja(Loja $entidadeLoja){
		try{
			...
			$stmt = $this->pdo->prepare("INSERT INTO loja (titulo_loja, descricao_loja, bool_ativo_loja) VALUES (:titulo_loja, :descricao_loja, :bool_ativo_loja);"
			);
	...
	function atualizaLoja(Loja $entidadeLoja, $id){
		try{
			...
			$stmt = $this->pdo->prepare("UPDATE loja SET titulo_loja = :titulo_loja, descricao_loja = :descricao_loja, bool_ativo_loja = :bool_ativo_loja WHERE id_loja = ".$id.";"
			);
			return $stmt->execute($param);
		}catch(PDOException $ex){
