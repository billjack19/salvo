...
faudittrailedit.validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		<?php } ?>
		<?php if ($audittrail_edit->action->Required) { ?>
			elm = this.getElements("x" + infix + "_action");
			if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
				return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $audittrail->action->caption(), $audittrail->action->RequiredErrorMessage)) ?>");
		<?php } ?>
...
<?php if ($audittrail->id->Visible) { // id ?>
	<div id="r_id" class="form-group row">
		<label id="elh_audittrail_id" class="<?php echo $audittrail_edit->LeftColumnClass ?>"><?php echo $audittrail->id->caption() ?><?php echo ($audittrail->id->Required) ? $Language->Phrase("FieldRequiredIndicator") : "" ?></label>
...
<?php if ($audittrail->action->Visible) { // action ?>
	<div id="r_action" class="form-group row">
		<label id="elh_audittrail_action" for="x_action" class="<?php echo $audittrail_edit->LeftColumnClass ?>"><?php echo $audittrail->action->caption() ?><?php echo ($audittrail->action->Required) ? $Language->Phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="text" data-table="audittrail" data-field="x_action" name="x_action" id="x_action" size="30" maxlength="80" placeholder="<?php echo HtmlEncode($audittrail->action->getPlaceHolder()) ?>" value="<?php echo $audittrail->action->EditValue ?>"<?php echo $audittrail->action->editAttributes() ?>>
</span>
