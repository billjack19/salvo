...
class usuario extends DbTable
{
	...
	public function __construct()
	{
		...
		// id_usuario
		$this->id_usuario = new DbField('usuario', 'usuario', 'x_id_usuario', 'id_usuario', '`id_usuario`', '`id_usuario`', 3, -1, FALSE, '`id_usuario`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// nome_usuario
		$this->nome_usuario = new DbField('usuario', 'usuario', 'x_nome_usuario', 'nome_usuario', '`nome_usuario`', '`nome_usuario`', 200, -1, FALSE, '`nome_usuario`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->nome_usuario->Sortable = TRUE; // Allow sort
		$this->fields['nome_usuario'] = &$this->nome_usuario;

		// login_usuario
		$this->login_usuario = new DbField('usuario', 'usuario', 'x_login_usuario', 'login_usuario', '`login_usuario`', '`login_usuario`', 200, -1, FALSE, '`login_usuario`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->login_usuario->Sortable = TRUE; // Allow sort
		$this->fields['login_usuario'] = &$this->login_usuario;

		// senha_usuario
		$this->senha_usuario = new DbField('usuario', 'usuario', 'x_senha_usuario', 'senha_usuario', '`senha_usuario`', '`senha_usuario`', 200, -1, FALSE, '`senha_usuario`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->senha_usuario->Sortable = TRUE; // Allow sort
		$this->fields['senha_usuario'] = &$this->senha_usuario;

		// nivel_acesso_id
		$this->nivel_acesso_id = new DbField('usuario', 'usuario', 'x_nivel_acesso_id', 'nivel_acesso_id', '`nivel_acesso_id`', '`nivel_acesso_id`', 3, -1, FALSE, '`nivel_acesso_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// bool_ativo_usuario
		$this->bool_ativo_usuario = new DbField('usuario', 'usuario', 'x_bool_ativo_usuario', 'bool_ativo_usuario', '`bool_ativo_usuario`', '`bool_ativo_usuario`', 3, -1, FALSE, '`bool_ativo_usuario`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->bool_ativo_usuario->Sortable = TRUE; // Allow sort
		$this->bool_ativo_usuario->DefaultErrorMessage = $Language->Phrase("IncorrectInteger");
		$this->fields['bool_ativo_usuario'] = &$this->bool_ativo_usuario;
	}
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->nome_usuario->DbValue = $row['nome_usuario'];
		$this->login_usuario->DbValue = $row['login_usuario'];
		$this->senha_usuario->DbValue = $row['senha_usuario'];
		$this->nivel_acesso_id->DbValue = $row['nivel_acesso_id'];
		$this->bool_ativo_usuario->DbValue = $row['bool_ativo_usuario'];
	}
	...
	public function getRecordFilter($row = NULL)
	{
		...
		$val = is_array($row) ? (array_key_exists('id_usuario', $row) ? $row['id_usuario'] : NULL) : $this->id_usuario->CurrentValue;
		if (!is_numeric($val))
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->nome_usuario->setDbValue($rs->fields('nome_usuario'));
		$this->login_usuario->setDbValue($rs->fields('login_usuario'));
		$this->senha_usuario->setDbValue($rs->fields('senha_usuario'));
		$this->nivel_acesso_id->setDbValue($rs->fields('nivel_acesso_id'));
		$this->bool_ativo_usuario->setDbValue($rs->fields('bool_ativo_usuario'));
	}
	...
	public function renderEditRow()
	{
		...
		$this->nome_usuario->EditValue = $this->nome_usuario->CurrentValue;
		...
		$this->senha_usuario->EditValue = $this->senha_usuario->CurrentValue;
		...
		$this->bool_ativo_usuario->EditValue = $this->bool_ativo_usuario->CurrentValue;
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					if ($this->id_usuario->Exportable)
						$doc->exportCaption($this->id_usuario);
					if ($this->nome_usuario->Exportable)
						$doc->exportCaption($this->nome_usuario);
					if ($this->login_usuario->Exportable)
						$doc->exportCaption($this->login_usuario);
					if ($this->senha_usuario->Exportable)
						$doc->exportCaption($this->senha_usuario);
					if ($this->nivel_acesso_id->Exportable)
						$doc->exportCaption($this->nivel_acesso_id);
					if ($this->bool_ativo_usuario->Exportable)
						$doc->exportCaption($this->bool_ativo_usuario);
				} else {
					if ($this->id_usuario->Exportable)
						$doc->exportCaption($this->id_usuario);
					if ($this->nome_usuario->Exportable)
			...
			if ($recCnt >= $startRec) {
				...
				if (!$doc->ExportCustom) {
					...
					if ($exportPageType == "view") {
						...
						if ($this->nome_usuario->Exportable)
							$doc->exportField($this->nome_usuario);
						if ($this->login_usuario->Exportable)
							$doc->exportField($this->login_usuario);
						if ($this->senha_usuario->Exportable)
							$doc->exportField($this->senha_usuario);
						if ($this->nivel_acesso_id->Exportable)
							$doc->exportField($this->nivel_acesso_id);
						if ($this->bool_ativo_usuario->Exportable)
							$doc->exportField($this->bool_ativo_usuario);
					} else {
						if ($this->id_usuario->Exportable)
							$doc->exportField($this->id_usuario);
						if ($this->nome_usuario->Exportable)
