...
class audittrail extends DbTable
{
	...
	public function __construct()
	{
		...
		// id
		$this->id = new DbField('audittrail', 'audittrail', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// datetime
		$this->datetime = new DbField('audittrail', 'audittrail', 'x_datetime', 'datetime', '`datetime`', CastDateFieldForLike('`datetime`', 0, "DB"), 135, 0, FALSE, '`datetime`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// script
		$this->script = new DbField('audittrail', 'audittrail', 'x_script', 'script', '`script`', '`script`', 200, -1, FALSE, '`script`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->script->Sortable = TRUE; // Allow sort
		$this->fields['script'] = &$this->script;

		// user
		$this->user = new DbField('audittrail', 'audittrail', 'x_user', 'user', '`user`', '`user`', 200, -1, FALSE, '`user`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->user->Sortable = TRUE; // Allow sort
		$this->fields['user'] = &$this->user;

		// action
		$this->action = new DbField('audittrail', 'audittrail', 'x_action', 'action', '`action`', '`action`', 200, -1, FALSE, '`action`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->action->Sortable = TRUE; // Allow sort
		$this->fields['action'] = &$this->action;

		// table
		$this->_table = new DbField('audittrail', 'audittrail', 'x__table', 'table', '`table`', '`table`', 200, -1, FALSE, '`table`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->_table->Sortable = TRUE; // Allow sort
		$this->fields['table'] = &$this->_table;

		// field
		$this->field = new DbField('audittrail', 'audittrail', 'x_field', 'field', '`field`', '`field`', 200, -1, FALSE, '`field`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->field->Sortable = TRUE; // Allow sort
		$this->fields['field'] = &$this->field;

		// keyvalue
		$this->keyvalue = new DbField('audittrail', 'audittrail', 'x_keyvalue', 'keyvalue', '`keyvalue`', '`keyvalue`', 201, -1, FALSE, '`keyvalue`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->keyvalue->Sortable = TRUE; // Allow sort
		$this->fields['keyvalue'] = &$this->keyvalue;

		// oldvalue
		$this->oldvalue = new DbField('audittrail', 'audittrail', 'x_oldvalue', 'oldvalue', '`oldvalue`', '`oldvalue`', 201, -1, FALSE, '`oldvalue`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->oldvalue->Sortable = TRUE; // Allow sort
		$this->fields['oldvalue'] = &$this->oldvalue;

		// newvalue
		$this->newvalue = new DbField('audittrail', 'audittrail', 'x_newvalue', 'newvalue', '`newvalue`', '`newvalue`', 201, -1, FALSE, '`newvalue`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
	...
	public function getViewUrl($parm = "")
	{
		if ($parm <> "")
			...
			$url = $this->keyUrl("audittrailview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	public function renderEditRow()
	{
		...
		$this->action->PlaceHolder = RemoveHtml($this->action->caption());
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					...
					if ($this->datetime->Exportable)
						$doc->exportCaption($this->datetime);
					if ($this->script->Exportable)
						$doc->exportCaption($this->script);
					if ($this->user->Exportable)
						$doc->exportCaption($this->user);
					if ($this->action->Exportable)
						$doc->exportCaption($this->action);
					if ($this->_table->Exportable)
						$doc->exportCaption($this->_table);
					if ($this->field->Exportable)
						$doc->exportCaption($this->field);
					if ($this->keyvalue->Exportable)
						$doc->exportCaption($this->keyvalue);
					if ($this->oldvalue->Exportable)
						$doc->exportCaption($this->oldvalue);
					if ($this->newvalue->Exportable)
