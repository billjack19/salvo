...
class grupo extends DbTable
{
	...
	public $imagem_grupo;
	public $data_atualizacao_grupo;
	public $usuario_id;
	public $bool_ativo_grupo;
	...
	public function __construct()
	{
		...
		// id_grupo
		$this->id_grupo = new DbField('grupo', 'grupo', 'x_id_grupo', 'id_grupo', '`id_grupo`', '`id_grupo`', 3, -1, FALSE, '`id_grupo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// descricao_grupo
		$this->descricao_grupo = new DbField('grupo', 'grupo', 'x_descricao_grupo', 'descricao_grupo', '`descricao_grupo`', '`descricao_grupo`', 200, -1, FALSE, '`descricao_grupo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->descricao_grupo->Nullable = FALSE; // NOT NULL field
		$this->descricao_grupo->Required = TRUE; // Required field
		$this->descricao_grupo->Sortable = TRUE; // Allow sort
		$this->fields['descricao_grupo'] = &$this->descricao_grupo;

		// imagem_grupo
		$this->imagem_grupo = new DbField('grupo', 'grupo', 'x_imagem_grupo', 'imagem_grupo', '`imagem_grupo`', '`imagem_grupo`', 200, -1, FALSE, '`imagem_grupo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->imagem_grupo->Nullable = FALSE; // NOT NULL field
		$this->imagem_grupo->Required = TRUE; // Required field
		$this->imagem_grupo->Sortable = TRUE; // Allow sort
		$this->fields['imagem_grupo'] = &$this->imagem_grupo;

		// data_atualizacao_grupo
		$this->data_atualizacao_grupo = new DbField('grupo', 'grupo', 'x_data_atualizacao_grupo', 'data_atualizacao_grupo', '`data_atualizacao_grupo`', CastDateFieldForLike('`data_atualizacao_grupo`', 0, "DB"), 135, 0, FALSE, '`data_atualizacao_grupo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->data_atualizacao_grupo->Nullable = FALSE; // NOT NULL field
		$this->data_atualizacao_grupo->Required = TRUE; // Required field
		$this->data_atualizacao_grupo->Sortable = TRUE; // Allow sort
		$this->data_atualizacao_grupo->DefaultErrorMessage = str_replace("%s", $GLOBALS["DATE_FORMAT"], $Language->Phrase("IncorrectDate"));
		$this->fields['data_atualizacao_grupo'] = &$this->data_atualizacao_grupo;

		// usuario_id
		$this->usuario_id = new DbField('grupo', 'grupo', 'x_usuario_id', 'usuario_id', '`usuario_id`', '`usuario_id`', 3, -1, FALSE, '`usuario_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// bool_ativo_grupo
		$this->bool_ativo_grupo = new DbField('grupo', 'grupo', 'x_bool_ativo_grupo', 'bool_ativo_grupo', '`bool_ativo_grupo`', '`bool_ativo_grupo`', 3, -1, FALSE, '`bool_ativo_grupo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->bool_ativo_grupo->Nullable = FALSE; // NOT NULL field
		$this->bool_ativo_grupo->Required = TRUE; // Required field
		$this->bool_ativo_grupo->Sortable = TRUE; // Allow sort
		$this->bool_ativo_grupo->DefaultErrorMessage = $Language->Phrase("IncorrectInteger");
		$this->fields['bool_ativo_grupo'] = &$this->bool_ativo_grupo;
	}
	...
	public function insert(&$rs)
	{
		...
		if ($success) {
			...
			$this->id_grupo->setDbValue($conn->insert_ID());
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->descricao_grupo->DbValue = $row['descricao_grupo'];
		$this->imagem_grupo->DbValue = $row['imagem_grupo'];
		$this->data_atualizacao_grupo->DbValue = $row['data_atualizacao_grupo'];
		$this->usuario_id->DbValue = $row['usuario_id'];
		$this->bool_ativo_grupo->DbValue = $row['bool_ativo_grupo'];
	}
	...
	public function getRecordFilter($row = NULL)
	{
		...
		$val = is_array($row) ? (array_key_exists('id_grupo', $row) ? $row['id_grupo'] : NULL) : $this->id_grupo->CurrentValue;
		if (!is_numeric($val))
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "grupoedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "grupoadd.php")
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("grupoview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("grupoview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "grupoadd.php?" . $this->getUrlParm($parm);
		else
			$url = "grupoadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("grupoedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("grupoadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("grupodelete.php", $this->getUrlParm());
	}
	...
	public function keyUrl($url, $parm = "")
	{
		...
		if ($this->id_grupo->CurrentValue != NULL) {
			$url .= "id_grupo=" . urlencode($this->id_grupo->CurrentValue);
		} else {
	...
	public function getRecordKeys()
	{
		...
		if (Param("key_m") !== NULL) {
			...
			if (Param("id_grupo") !== NULL)
				$arKeys[] = Param("id_grupo");
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->descricao_grupo->setDbValue($rs->fields('descricao_grupo'));
		$this->imagem_grupo->setDbValue($rs->fields('imagem_grupo'));
		$this->data_atualizacao_grupo->setDbValue($rs->fields('data_atualizacao_grupo'));
		$this->usuario_id->setDbValue($rs->fields('usuario_id'));
		$this->bool_ativo_grupo->setDbValue($rs->fields('bool_ativo_grupo'));
	}
	...
	public function renderEditRow()
	{
		...
		$this->descricao_grupo->EditValue = $this->descricao_grupo->CurrentValue;
		...
		$this->imagem_grupo->EditValue = $this->imagem_grupo->CurrentValue;
		...
		$this->data_atualizacao_grupo->EditValue = FormatDateTime($this->data_atualizacao_grupo->CurrentValue, 8);
		...
		$this->bool_ativo_grupo->EditValue = $this->bool_ativo_grupo->CurrentValue;
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					if ($this->id_grupo->Exportable)
						$doc->exportCaption($this->id_grupo);
					if ($this->descricao_grupo->Exportable)
						$doc->exportCaption($this->descricao_grupo);
					if ($this->imagem_grupo->Exportable)
						$doc->exportCaption($this->imagem_grupo);
					if ($this->data_atualizacao_grupo->Exportable)
						$doc->exportCaption($this->data_atualizacao_grupo);
					if ($this->usuario_id->Exportable)
						$doc->exportCaption($this->usuario_id);
					if ($this->bool_ativo_grupo->Exportable)
						$doc->exportCaption($this->bool_ativo_grupo);
				} else {
					if ($this->id_grupo->Exportable)
						$doc->exportCaption($this->id_grupo);
					if ($this->descricao_grupo->Exportable)
			...
			if ($recCnt >= $startRec) {
				...
				if (!$doc->ExportCustom) {
					...
					if ($exportPageType == "view") {
						...
						if ($this->descricao_grupo->Exportable)
							$doc->exportField($this->descricao_grupo);
						if ($this->imagem_grupo->Exportable)
							$doc->exportField($this->imagem_grupo);
						if ($this->data_atualizacao_grupo->Exportable)
							$doc->exportField($this->data_atualizacao_grupo);
						if ($this->usuario_id->Exportable)
							$doc->exportField($this->usuario_id);
						if ($this->bool_ativo_grupo->Exportable)
							$doc->exportField($this->bool_ativo_grupo);
					} else {
						if ($this->id_grupo->Exportable)
							$doc->exportField($this->id_grupo);
						if ($this->descricao_grupo->Exportable)
