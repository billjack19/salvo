...
// Create page object
$notificacoes_edit = new notificacoes_edit();

// Run the page
$notificacoes_edit->run();
...
var fnotificacoesedit = currentForm = new ew.Form("fnotificacoesedit", "edit");

// Validate form
fnotificacoesedit.validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		<?php if ($notificacoes_edit->id_notificacoes->Required) { ?>
			elm = this.getElements("x" + infix + "_id_notificacoes");
			if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
				return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $notificacoes->id_notificacoes->caption(), $notificacoes->id_notificacoes->RequiredErrorMessage)) ?>");
		<?php } ?>
		<?php if ($notificacoes_edit->descricao_notificacoes->Required) { ?>
		...
		<?php } ?>
		<?php if ($notificacoes_edit->usuario_atuador_notificacoes->Required) { ?>
		...
		<?php } ?>
		<?php if ($notificacoes_edit->usuaio_requerente_notificacoes->Required) { ?>
		...
		<?php } ?>
		<?php if ($notificacoes_edit->tipo_alteracao_notificacoes->Required) { ?>
		...
		<?php } ?>
		<?php if ($notificacoes_edit->notificacoes_config_id->Required) { ?>
		...
		<?php if ($notificacoes_edit->bool_view_notificacoes->Required) { ?>
		...
		<?php if ($notificacoes_edit->data_atualizacao_notificacoes->Required) { ?>
		...
		<?php if ($notificacoes_edit->bool_ativo_notificacoes->Required) { ?>
...
fnotificacoesedit.Form_CustomValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
...
fnotificacoesedit.validateRequired = <?php echo json_encode(CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fnotificacoesedit.lists["x_tipo_alteracao_notificacoes"] = <?php echo $notificacoes_edit->tipo_alteracao_notificacoes->Lookup->toClientList() ?>;
fnotificacoesedit.lists["x_tipo_alteracao_notificacoes"].options = <?php echo JsonEncode($notificacoes_edit->tipo_alteracao_notificacoes->options(FALSE, TRUE)) ?>;
...
<?php $notificacoes_edit->showPageHeader(); ?>
<?php
$notificacoes_edit->showMessage();
?>
<form name="fnotificacoesedit" id="fnotificacoesedit" class="<?php echo $notificacoes_edit->FormClassName ?>" action="<?php echo CurrentPageName() ?>" method="post">
<?php if ($notificacoes_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo TOKEN_NAME ?>" value="<?php echo $notificacoes_edit->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="notificacoes">
<input type="hidden" name="action" id="action" value="update">
<input type="hidden" name="modal" value="<?php echo (int)$notificacoes_edit->IsModal ?>">
<div class="ew-edit-div"><!-- page* -->
<?php if ($notificacoes->id_notificacoes->Visible) { // id_notificacoes ?>
	<div id="r_id_notificacoes" class="form-group row">
		<label id="elh_notificacoes_id_notificacoes" class="<?php echo $notificacoes_edit->LeftColumnClass ?>"><?php echo $notificacoes->id_notificacoes->caption() ?><?php echo ($notificacoes->id_notificacoes->Required) ? $Language->Phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $notificacoes_edit->RightColumnClass ?>"><div<?php echo $notificacoes->id_notificacoes->cellAttributes() ?>>
<span id="el_notificacoes_id_notificacoes">
<span<?php echo $notificacoes->id_notificacoes->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?php echo RemoveHtml($notificacoes->id_notificacoes->EditValue) ?>"></span>
</span>
<input type="hidden" data-table="notificacoes" data-field="x_id_notificacoes" name="x_id_notificacoes" id="x_id_notificacoes" value="<?php echo HtmlEncode($notificacoes->id_notificacoes->CurrentValue) ?>">
<?php echo $notificacoes->id_notificacoes->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($notificacoes->descricao_notificacoes->Visible) { // descricao_notificacoes ?>
	<div id="r_descricao_notificacoes" class="form-group row">
		<label id="elh_notificacoes_descricao_notificacoes" for="x_descricao_notificacoes" class="<?php echo $notificacoes_edit->LeftColumnClass ?>"><?php echo $notificacoes->descricao_notificacoes->caption() ?><?php echo ($notificacoes->descricao_notificacoes->Required) ? $Language->Phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $notificacoes_edit->RightColumnClass ?>"><div<?php echo $notificacoes->descricao_notificacoes->cellAttributes() ?>>
...
<?php if ($notificacoes->usuario_atuador_notificacoes->Visible) { // usuario_atuador_notificacoes ?>
	<div id="r_usuario_atuador_notificacoes" class="form-group row">
		<label id="elh_notificacoes_usuario_atuador_notificacoes" for="x_usuario_atuador_notificacoes" class="<?php echo $notificacoes_edit->LeftColumnClass ?>"><?php echo $notificacoes->usuario_atuador_notificacoes->caption() ?><?php echo ($notificacoes->usuario_atuador_notificacoes->Required) ? $Language->Phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $notificacoes_edit->RightColumnClass ?>"><div<?php echo $notificacoes->usuario_atuador_notificacoes->cellAttributes() ?>>
...
<?php if ($notificacoes->usuaio_requerente_notificacoes->Visible) { // usuaio_requerente_notificacoes ?>
	<div id="r_usuaio_requerente_notificacoes" class="form-group row">
		<label id="elh_notificacoes_usuaio_requerente_notificacoes" for="x_usuaio_requerente_notificacoes" class="<?php echo $notificacoes_edit->LeftColumnClass ?>"><?php echo $notificacoes->usuaio_requerente_notificacoes->caption() ?><?php echo ($notificacoes->usuaio_requerente_notificacoes->Required) ? $Language->Phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $notificacoes_edit->RightColumnClass ?>"><div<?php echo $notificacoes->usuaio_requerente_notificacoes->cellAttributes() ?>>
...
<?php if ($notificacoes->tipo_alteracao_notificacoes->Visible) { // tipo_alteracao_notificacoes ?>
	<div id="r_tipo_alteracao_notificacoes" class="form-group row">
		<label id="elh_notificacoes_tipo_alteracao_notificacoes" class="<?php echo $notificacoes_edit->LeftColumnClass ?>"><?php echo $notificacoes->tipo_alteracao_notificacoes->caption() ?><?php echo ($notificacoes->tipo_alteracao_notificacoes->Required) ? $Language->Phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $notificacoes_edit->RightColumnClass ?>"><div<?php echo $notificacoes->tipo_alteracao_notificacoes->cellAttributes() ?>>
...
<?php if ($notificacoes->notificacoes_config_id->Visible) { // notificacoes_config_id ?>
	<div id="r_notificacoes_config_id" class="form-group row">
		<label id="elh_notificacoes_notificacoes_config_id" for="x_notificacoes_config_id" class="<?php echo $notificacoes_edit->LeftColumnClass ?>"><?php echo $notificacoes->notificacoes_config_id->caption() ?><?php echo ($notificacoes->notificacoes_config_id->Required) ? $Language->Phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $notificacoes_edit->RightColumnClass ?>"><div<?php echo $notificacoes->notificacoes_config_id->cellAttributes() ?>>
...
<?php if ($notificacoes->bool_view_notificacoes->Visible) { // bool_view_notificacoes ?>
	<div id="r_bool_view_notificacoes" class="form-group row">
		<label id="elh_notificacoes_bool_view_notificacoes" for="x_bool_view_notificacoes" class="<?php echo $notificacoes_edit->LeftColumnClass ?>"><?php echo $notificacoes->bool_view_notificacoes->caption() ?><?php echo ($notificacoes->bool_view_notificacoes->Required) ? $Language->Phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $notificacoes_edit->RightColumnClass ?>"><div<?php echo $notificacoes->bool_view_notificacoes->cellAttributes() ?>>
...
<?php if ($notificacoes->data_atualizacao_notificacoes->Visible) { // data_atualizacao_notificacoes ?>
	<div id="r_data_atualizacao_notificacoes" class="form-group row">
		<label id="elh_notificacoes_data_atualizacao_notificacoes" for="x_data_atualizacao_notificacoes" class="<?php echo $notificacoes_edit->LeftColumnClass ?>"><?php echo $notificacoes->data_atualizacao_notificacoes->caption() ?><?php echo ($notificacoes->data_atualizacao_notificacoes->Required) ? $Language->Phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $notificacoes_edit->RightColumnClass ?>"><div<?php echo $notificacoes->data_atualizacao_notificacoes->cellAttributes() ?>>
...
<?php if ($notificacoes->bool_ativo_notificacoes->Visible) { // bool_ativo_notificacoes ?>
	<div id="r_bool_ativo_notificacoes" class="form-group row">
		<label id="elh_notificacoes_bool_ativo_notificacoes" for="x_bool_ativo_notificacoes" class="<?php echo $notificacoes_edit->LeftColumnClass ?>"><?php echo $notificacoes->bool_ativo_notificacoes->caption() ?><?php echo ($notificacoes->bool_ativo_notificacoes->Required) ? $Language->Phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $notificacoes_edit->RightColumnClass ?>"><div<?php echo $notificacoes->bool_ativo_notificacoes->cellAttributes() ?>>
...
<?php if (!$notificacoes_edit->IsModal) { ?>
<div class="form-group row"><!-- buttons .form-group -->
	<div class="<?php echo $notificacoes_edit->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit"><?php echo $Language->Phrase("SaveBtn") ?></button>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-href="<?php echo $notificacoes_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
</div><!-- /buttons .form-group -->
<?php } ?>
</form>
<?php
$notificacoes_edit->showPageFooter();
if (DEBUG_ENABLED)
