...
function buscarSupermercado(){
	...
	if(localStorage.selectSupermecado){
		$("#comboSupermecado").html(localStorage.getItem("selectSupermecado"));
	} else {
		$.ajax({
			...
			try{
				...
				if (dataObj[0].id_supermercado != undefined) {
					...
					localStorage.setItem("selectSupermecado", selectSupermecado);
	...
	if (id_pedido_Global != 0) {
		$.ajax({
			...
			try{
				data = JSON.parse(data)
				if (data.length == 0) {
					$("#divListaProd").html("<h2>Nenhum item nesse pedido</h2>");
				} else {
					var resultado = "<table>";
					var descrcaoItem = "";
					/* Setar cabecario da tabela */
					for (var i = 0; i < data.length; i++) {
						descrcaoItem = 	data[i].quantidade_item 			+ " " +
										data[i].sigla_unidade_medida 		+ " " +
										data[i].descricao_item 				+ " " +
										data[i].descricao_marca 			+ " " +
										data[i].especificacao_equivalentes;

						resultado += "<tr>"
								  +  	"<td>" + descrcaoItem + "</td>"
								  + 	"<td>"
								  + 		"<button class='btn btn-danger' onclick='apagarItemPedido(" + data[i].id_pedido_item + ")'>"
					...
					resultado += "</table>";
					$("#divListaProd").html(resultado);
				}
			} catch(error){
				console.log("Falha na conversão do objeto JSON: " + error);
			}
		});
	} else {
		$("#divListaProd").html("<h2>Não tem nenhum pedido selecionado</h2>");
	}
}
...
function buscarUnidadeMedida(){
	if(localStorage.selectUnidadeMedida){
		$("#selectUnidadeMedDiv").html(localStorage.getItem("selectUnidadeMedida"));
	} else {
		$.ajax({
			...
			try{
				...
				localStorage.setItem("selectUnidadeMedida", select);
		...
		try{
			...
			if(data[0].debug != "OK") console.error("Error: " + data.debug);
				...
				var inputDataList = "<div class=\"input-group\">"
								  ...
								  +			" onchange='buscarProdutoMarca(this.value)' "
								  // + 		" onblur='selecionaOs(this.value)'"
								  +			" required disabled>"
...
function buscarProdutoMarca(id_marca){
	var resultado = "";
	$.ajax({
		url: ip_Global + "controllers/controllerProduto.php",
		type: "POST",
		...
		try{
			...
			for (var i = 0; i < data.length; i++) {
				resultado += 	"<tr>"
						  ...
						  + 		"</td>"
						  + 		"<td>"
						  + 			"<button class='btn btn-info' onclick='selecionarItem("+data[i].id_item+")'>"
			...
			console.log("Falha ao tentar converter resultado em objeto");
			resultado = "<h2>Nenhum Regitro Encotrando</h2>";
		}
		$("#produtodMarcaDiv")[0].style.display = data.length > 0 ? "block" : "none";
		$("#tabelaProdutoMarcaDiv").html(resultado);
	});
}
