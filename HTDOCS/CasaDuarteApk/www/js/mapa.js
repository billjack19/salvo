	...
	directionsService.route(request, function(result, status) {
		if (status == google.maps.DirectionsStatus.OK) {
			...
			/*for (var j = 0; j < rotas.length; j++) {
				...
				igualEntregas = false;
				if (arrayPontosEntregas.length == rotas[j].endEntrega.length) {
					for (var k = 0; k < rotas[j].endEntrega.length; k++) {
						localizacao = rotas[j].endEntrega[k].endEntrega.split(",");
						if (
							localizacao[0] == arrayPontosEntregas[k].location.location.lat() && 
							localizacao[1] == arrayPontosEntregas[k].location.location.lng()
						) { igualEntregas = true; }
						else { igualEntregas = false; k = rotas[j].endEntrega.length; }
					}
					if (igualEntregas) {
...
function tratandoResultado(objMap, cor, rotaDetalhe, objetoEntrega){//cor, //, numeroDeRotas
	...
	for (var i = 0; i < paradas.length; i++) {
		...
		if (i != paradas.length - 1) {
			...
			if (objetoEntrega[indiceRealEnterga].bool_entrega == 1) {
				corReference = corConfirmacao;
			} else {
				arrayCodigos.push(objetoEntrega[indiceRealEnterga].codigo);
			...
			arrayIndiceMarcador[arrayIndiceMarcador.length - 1].push(
				adicionarMarcador(
					...
					objetoEntrega[indiceRealEnterga]
				)
			);
			...
			locaisDeEntregaIndice = {
				...
				codigo: objetoEntrega[indiceRealEnterga].codigo,
				bool_entrega: objetoEntrega[indiceRealEnterga].bool_entrega
			};
...
function adicionarMarcador(latitude, longitude, img, titulo, objetoEntrega){
	/* var contentString = retornarJanelaInfo(titulo); */
	...
	Marker.addListener('click', function() {
		if (
			...
			alert("Desabilitando tela cheia!");
		}
...
function descreverLocaisEnterga(array, rotaDetalhe, cor, numeroDaRota){
	...
	for (var i = 0; i < array.length; i++) {
		...
		tabela += array[i].bool_entrega == 0 ? 
			retornaBotao("check", array[i].pedido, rotaDetalhe.codigo, array[i].codigo) : 
			retornaBotao("times", array[i].pedido, rotaDetalhe.codigo, array[i].codigo);
...
function retornaBotao(tipo, pedido, entrega, codigoEntrega){
	var resultado = "";
	if (tipo == "check") {
		resultado += "<butto class='btn btn-success btn-block' onclick='confirmaPedido(\""+pedido+"\", "+entrega+", "+codigoEntrega+")'>";
		...
		resultado += 	"<i class='fa fa-check'></i>&nbsp;&nbsp;Confirmar"
		resultado += "</button>";*/
		resultado += "<butto class='btn btn-danger btn-block' onclick='cancelarPedido(\""+pedido+"\", "+entrega+", "+codigoEntrega+")'>";
		resultado += 	"<i class='fa fa-check'></i>&nbsp;&nbsp;Desconfirmar"
		resultado += "</button>";
	}
	return resultado;
}
	...
	for (var i = 0; i < MarcadoreEntregas.length; i++) {
		if (MarcadoreEntregas[i].Marker.codigo != undefined) {
			$.ajax({
				...
				var corPacote = objData.bool_entrega == 0 ? arrayDeCores[0].cor : corConfirmacao;
