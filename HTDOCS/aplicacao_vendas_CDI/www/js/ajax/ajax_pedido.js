	...
	$.ajax({
		...
		if (data.length == 0) {
			...
			if ($("#fichaPesquisa").val() == 0) {
				...
				motarSubMenu += "<input type='number' id='ficha' onkeyup='truncaDez(this.value, \"ficha\");' class='form-control'>";
			} else {
				var numeroPesquisadoFicha = $("#fichaPesquisa").val();
				motarSubMenu += "<h4>Ficha</h4>";
				motarSubMenu += "<input type='number' value='"+numeroPesquisadoFicha+"' id='ficha' onkeyup='truncaDez(this.value, \"ficha\");' class='form-control'>";
			}
...
function montarListaItem(lancPedidoId, desconto){
	var motarSubMenu = "";
	...
	$.ajax({
		...
		for(i in data){
			...
			motarSubMenu += "<button class='btn-block btn btn-primary' onclick='montarListaItem("+lancPedidoId+", "+desconto+")'>";
			motarSubMenu += "<i class='fa fa-refresh' aria-hidden='true'></i>&nbsp;Atualizar"
			motarSubMenu += "</button>";
	...
	// console.log(urlWebService+"ItemWs/listarLancPedidoItem/"+lancPedidoId+"/id"+parametrosWebService);
	$.ajax({
		type: 'GET',
		...
		montarListaItens += "<button class='btn btn-success' onclick='setStatus(\"pagina3\");montarViewAdicionaItem("+desconto+");'>";
		...
		if (data.length == 0) {
			...
			valorTotalReal = parseFloat(valorTotalItem) - parseFloat(desconto);

			valorTotalItem = formataValorParaImprimir(valorTotalItem);
			montarListaItens += "<tr><td colspan='3'></td>";
			montarListaItens += "<td align='right'>Valor Total&nbsp;</td>";
			...
			desconto = formataValorParaImprimir(desconto);
			montarListaItens += "<tr><td colspan='3'></td>";
			montarListaItens += "<td align='right'>Valor Desconto&nbsp;</td>";
			montarListaItens += "<td align='left'>";
			montarListaItens += "<span id='valorTotalPedidoSpan'>"+desconto+"</span>";
			montarListaItens += "</td>";
			montarListaItens += "<td></td>";
			montarListaItens += "</tr>";


			valorTotalReal = formataValorParaImprimir(valorTotalReal);
			montarListaItens += "<tr><td colspan='3'></td>";
			montarListaItens += "<td align='right'>Valor Pago&nbsp;</td>";
			montarListaItens += "<td align='left'>";
			montarListaItens += "<span id='valorTotalPedidoSpan'>"+valorTotalReal+"</span>";
			montarListaItens += "</td>";
			montarListaItens += "<td></td>";
			montarListaItens += "</tr>";
...
function montarListaItemVisualizar(lancPedidoId, documento, desconto){
	var motarSubMenu = "";
	var valorTotalPedido = 0;
	var documentoGeral = "";
	var emissaoItem = 0;
	...
	$.ajax({
		type: 'GET',
		url: urlWebService+"ItemWs/listarLancPedidoItemDoc/"+documento+parametrosWebService,
		contentType: "application/json",
		jsonpCallback: "localJsonpCallback"
	}).done( function(data){
		var valorTotalItem = 0;
