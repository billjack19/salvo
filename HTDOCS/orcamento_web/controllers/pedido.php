...
if(
	...
	foreach ($verifica as $dados) {
		...
		// echo $dados['WEB_BANCO_DADOS']."\n";
		// echo $dados['WEB_USUARIO_BD']."\n";
		// echo $dados['WEB_SENHA_BD']."\n";
		// echo $dados['WEB_NOME_BD']."\n";
...
if($validador == 0) echo "erro Validador";
...
if (!empty($_POST['adicionarPedidoItem'])) {
	...
	if($documento == "0"){
		...
		$sql = "INSERT INTO lanc_marketing
				(
				filial, documento, cliente, razaosocial, total, endereco, numero, bairro, cidade, estado, cep, telefone, emissao, flagcancelada, usuarioatualizacao, pagamento, observacao, usuarioatualizacao_alteracao, vendedor, contato
				) VALUES (
				$filial, '$documento', $cliente, '$razaosocial', $total, '$endereco', '$numero', '$bairro', '$cidade', '$estado', '$cep', '$telefone', '$emissao', $flagcancelada, '$usuarioatualizacao', '$pagamento', '$observacao', '$usuarioatualizacao_alteracao', $vendedor, '$contato'
			)";
	} else {
	...
	$lanc_marketing->set( $contato, 			'contato'	 	 );
