...
function botaoFixoEsquerda(onde){
	...
	if (onde == "principal") {
		...
		botao += "</button>";
	}else if (onde == "addPedido" || onde == "menu") {
		...
		botao += "<button class='btn btn-primary' onclick='chamarTelaDeMenu(\""+data_Global+"\")'>";
		botao += 		"<i class='fa fa-list'></i> Menu";
		botao += "</button>";
	}
...
function botaoFixoDireita(onde){
	...
	if (onde == "principal") {
		...
		botao += "<button class='btn btn-success' onclick='montarTelaPrincipal(\""+data_Global+"\")'>";
		botao += 		"<i class='fa fa-shopping-cart'></i> Orçamento";
		botao += "</button>";
	}
