...
class siteDAO{
	...
	function cadastraSite(Site $entidadeSite){
		try{
			...
			$stmt = $this->pdo->prepare("INSERT INTO site ( NOME_EMPRESA, NOME_CIDADE, ESTADO, FONE, FONE_APP, EMAIL, sendusername, sendpassword, smtpserver, smtpserverport, MailFrom, MailTo, MailCc )  VALUES ( :NOME_EMPRESA, :NOME_CIDADE, :ESTADO, :FONE, :FONE_APP, :EMAIL, :sendusername, :sendpassword, :smtpserver, :smtpserverport, :MailFrom, :MailTo, :MailCc );");
	...
	function atualizaSite(Site $entidadeSite ,$id){
		try{
			...
			$stmt = $this->pdo->prepare("UPDATE site SET NOME_EMPRESA = :NOME_EMPRESA, NOME_CIDADE = :NOME_CIDADE, ESTADO = :ESTADO, FONE = :FONE, FONE_APP = :FONE_APP, EMAIL = :EMAIL, sendusername = :sendusername, sendpassword = :sendpassword, smtpserver = :smtpserver, smtpserverport = :smtpserverport, MailFrom = :MailFrom, MailTo = :MailTo, MailCc = :MailCc WHERE ID_SITE = ".$id.";");
			return $stmt->execute($param);
		}catch(PDOException $ex){
