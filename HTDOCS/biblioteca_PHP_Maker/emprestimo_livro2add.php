...
$emprestimo_livro2_add = NULL; // Initialize page object first

class cemprestimo_livro2_add extends cemprestimo_livro2 {
	...
	// Page object name
	var $PageObjName = 'emprestimo_livro2_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["ID_EMPRESTIMO_LIVRO2"] != "") {
				...
				$this->setKey("ID_EMPRESTIMO_LIVRO2", $this->ID_EMPRESTIMO_LIVRO2->CurrentValue); // Set up key
			} else {
				$this->setKey("ID_EMPRESTIMO_LIVRO2", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "C": // Copy an existing record
				if (!$loaded) { // Record not loaded
					...
					$this->Page_Terminate("emprestimo_livro2list.php"); // No matching record, return to list
				}
				break;
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "emprestimo_livro2list.php")
						$sReturnUrl = $this->AddMasterUrl($sReturnUrl); // List page, return to List page with correct master key if necessary
					elseif (ew_GetPageName($sReturnUrl) == "emprestimo_livro2view.php")
	...
	function LoadDefaultValues() {
		$this->ID_EMPRESTIMO_LIVRO2->CurrentValue = NULL;
		$this->ID_EMPRESTIMO_LIVRO2->OldValue = $this->ID_EMPRESTIMO_LIVRO2->CurrentValue;
		...
		$this->DATA_INICIAL_EMPRESTIMO_LIVRO->CurrentValue = NULL;
		$this->DATA_INICIAL_EMPRESTIMO_LIVRO->OldValue = $this->DATA_INICIAL_EMPRESTIMO_LIVRO->CurrentValue;
		$this->DATA_FINAL_EMPRESTIMO_LIVRO->CurrentValue = NULL;
		$this->DATA_FINAL_EMPRESTIMO_LIVRO->OldValue = $this->DATA_FINAL_EMPRESTIMO_LIVRO->CurrentValue;
		$this->STATUS_EMPRESTIMO->CurrentValue = NULL;
		$this->STATUS_EMPRESTIMO->OldValue = $this->STATUS_EMPRESTIMO->CurrentValue;
	}
	...
	function NewRow() {
		...
		$row = array();
		$row['ID_EMPRESTIMO_LIVRO2'] = $this->ID_EMPRESTIMO_LIVRO2->CurrentValue;
		...
		$row['DATA_INICIAL_EMPRESTIMO_LIVRO'] = $this->DATA_INICIAL_EMPRESTIMO_LIVRO->CurrentValue;
		$row['DATA_FINAL_EMPRESTIMO_LIVRO'] = $this->DATA_FINAL_EMPRESTIMO_LIVRO->CurrentValue;
		$row['STATUS_EMPRESTIMO'] = $this->STATUS_EMPRESTIMO->CurrentValue;
		return $row;
	}
	...
	function AddRow($rsold = NULL) {
		...
		// ID_LIVRO
		$this->ID_LIVRO->SetDbValueDef($rsnew, $this->ID_LIVRO->CurrentValue, 0, FALSE);

		// ID_FUNCIONARIO
		$this->ID_FUNCIONARIO->SetDbValueDef($rsnew, $this->ID_FUNCIONARIO->CurrentValue, 0, FALSE);

		// ID_CADASTRO
		$this->ID_CADASTRO->SetDbValueDef($rsnew, $this->ID_CADASTRO->CurrentValue, 0, FALSE);

		// DATA_INICIAL_EMPRESTIMO_LIVRO
		$this->DATA_INICIAL_EMPRESTIMO_LIVRO->SetDbValueDef($rsnew, $this->DATA_INICIAL_EMPRESTIMO_LIVRO->CurrentValue, "", FALSE);

		// DATA_FINAL_EMPRESTIMO_LIVRO
		$this->DATA_FINAL_EMPRESTIMO_LIVRO->SetDbValueDef($rsnew, $this->DATA_FINAL_EMPRESTIMO_LIVRO->CurrentValue, "", FALSE);

		// STATUS_EMPRESTIMO
		$this->STATUS_EMPRESTIMO->SetDbValueDef($rsnew, $this->STATUS_EMPRESTIMO->CurrentValue, 0, FALSE);
...
// Create page object
if (!isset($emprestimo_livro2_add)) $emprestimo_livro2_add = new cemprestimo_livro2_add();

// Page init
$emprestimo_livro2_add->Page_Init();

// Page main
$emprestimo_livro2_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$emprestimo_livro2_add->Page_Render();
?>
...
var CurrentForm = femprestimo_livro2add = new ew_Form("femprestimo_livro2add", "add");

// Validate form
femprestimo_livro2add.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $emprestimo_livro2->ID_LIVRO->FldCaption(), $emprestimo_livro2->ID_LIVRO->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_ID_LIVRO");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($emprestimo_livro2->ID_LIVRO->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_ID_FUNCIONARIO");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $emprestimo_livro2->ID_FUNCIONARIO->FldCaption(), $emprestimo_livro2->ID_FUNCIONARIO->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_ID_FUNCIONARIO");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($emprestimo_livro2->ID_FUNCIONARIO->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_ID_CADASTRO");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $emprestimo_livro2->ID_CADASTRO->FldCaption(), $emprestimo_livro2->ID_CADASTRO->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_ID_CADASTRO");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($emprestimo_livro2->ID_CADASTRO->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_DATA_INICIAL_EMPRESTIMO_LIVRO");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $emprestimo_livro2->DATA_INICIAL_EMPRESTIMO_LIVRO->FldCaption(), $emprestimo_livro2->DATA_INICIAL_EMPRESTIMO_LIVRO->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_DATA_FINAL_EMPRESTIMO_LIVRO");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $emprestimo_livro2->DATA_FINAL_EMPRESTIMO_LIVRO->FldCaption(), $emprestimo_livro2->DATA_FINAL_EMPRESTIMO_LIVRO->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_STATUS_EMPRESTIMO");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $emprestimo_livro2->STATUS_EMPRESTIMO->FldCaption(), $emprestimo_livro2->STATUS_EMPRESTIMO->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_STATUS_EMPRESTIMO");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($emprestimo_livro2->STATUS_EMPRESTIMO->FldErrMsg()) ?>");
...
femprestimo_livro2add.Form_CustomValidate = 
...
femprestimo_livro2add.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
<?php $emprestimo_livro2_add->ShowPageHeader(); ?>
<?php
$emprestimo_livro2_add->ShowMessage();
?>
<form name="femprestimo_livro2add" id="femprestimo_livro2add" class="<?php echo $emprestimo_livro2_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($emprestimo_livro2_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $emprestimo_livro2_add->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="emprestimo_livro2">
<input type="hidden" name="a_add" id="a_add" value="A">
<input type="hidden" name="modal" value="<?php echo intval($emprestimo_livro2_add->IsModal) ?>">
<div class="ewAddDiv"><!-- page* -->
<?php if ($emprestimo_livro2->ID_LIVRO->Visible) { // ID_LIVRO ?>
	<div id="r_ID_LIVRO" class="form-group">
		<label id="elh_emprestimo_livro2_ID_LIVRO" for="x_ID_LIVRO" class="<?php echo $emprestimo_livro2_add->LeftColumnClass ?>"><?php echo $emprestimo_livro2->ID_LIVRO->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $emprestimo_livro2_add->RightColumnClass ?>"><div<?php echo $emprestimo_livro2->ID_LIVRO->CellAttributes() ?>>
<span id="el_emprestimo_livro2_ID_LIVRO">
<input type="text" data-table="emprestimo_livro2" data-field="x_ID_LIVRO" name="x_ID_LIVRO" id="x_ID_LIVRO" size="30" placeholder="<?php echo ew_HtmlEncode($emprestimo_livro2->ID_LIVRO->getPlaceHolder()) ?>" value="<?php echo $emprestimo_livro2->ID_LIVRO->EditValue ?>"<?php echo $emprestimo_livro2->ID_LIVRO->EditAttributes() ?>>
</span>
<?php echo $emprestimo_livro2->ID_LIVRO->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($emprestimo_livro2->ID_FUNCIONARIO->Visible) { // ID_FUNCIONARIO ?>
	<div id="r_ID_FUNCIONARIO" class="form-group">
		<label id="elh_emprestimo_livro2_ID_FUNCIONARIO" for="x_ID_FUNCIONARIO" class="<?php echo $emprestimo_livro2_add->LeftColumnClass ?>"><?php echo $emprestimo_livro2->ID_FUNCIONARIO->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $emprestimo_livro2_add->RightColumnClass ?>"><div<?php echo $emprestimo_livro2->ID_FUNCIONARIO->CellAttributes() ?>>
<span id="el_emprestimo_livro2_ID_FUNCIONARIO">
<input type="text" data-table="emprestimo_livro2" data-field="x_ID_FUNCIONARIO" name="x_ID_FUNCIONARIO" id="x_ID_FUNCIONARIO" size="30" placeholder="<?php echo ew_HtmlEncode($emprestimo_livro2->ID_FUNCIONARIO->getPlaceHolder()) ?>" value="<?php echo $emprestimo_livro2->ID_FUNCIONARIO->EditValue ?>"<?php echo $emprestimo_livro2->ID_FUNCIONARIO->EditAttributes() ?>>
</span>
<?php echo $emprestimo_livro2->ID_FUNCIONARIO->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($emprestimo_livro2->ID_CADASTRO->Visible) { // ID_CADASTRO ?>
	<div id="r_ID_CADASTRO" class="form-group">
		<label id="elh_emprestimo_livro2_ID_CADASTRO" for="x_ID_CADASTRO" class="<?php echo $emprestimo_livro2_add->LeftColumnClass ?>"><?php echo $emprestimo_livro2->ID_CADASTRO->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $emprestimo_livro2_add->RightColumnClass ?>"><div<?php echo $emprestimo_livro2->ID_CADASTRO->CellAttributes() ?>>
<span id="el_emprestimo_livro2_ID_CADASTRO">
<input type="text" data-table="emprestimo_livro2" data-field="x_ID_CADASTRO" name="x_ID_CADASTRO" id="x_ID_CADASTRO" size="30" placeholder="<?php echo ew_HtmlEncode($emprestimo_livro2->ID_CADASTRO->getPlaceHolder()) ?>" value="<?php echo $emprestimo_livro2->ID_CADASTRO->EditValue ?>"<?php echo $emprestimo_livro2->ID_CADASTRO->EditAttributes() ?>>
</span>
<?php echo $emprestimo_livro2->ID_CADASTRO->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($emprestimo_livro2->DATA_INICIAL_EMPRESTIMO_LIVRO->Visible) { // DATA_INICIAL_EMPRESTIMO_LIVRO ?>
	<div id="r_DATA_INICIAL_EMPRESTIMO_LIVRO" class="form-group">
		<label id="elh_emprestimo_livro2_DATA_INICIAL_EMPRESTIMO_LIVRO" for="x_DATA_INICIAL_EMPRESTIMO_LIVRO" class="<?php echo $emprestimo_livro2_add->LeftColumnClass ?>"><?php echo $emprestimo_livro2->DATA_INICIAL_EMPRESTIMO_LIVRO->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $emprestimo_livro2_add->RightColumnClass ?>"><div<?php echo $emprestimo_livro2->DATA_INICIAL_EMPRESTIMO_LIVRO->CellAttributes() ?>>
<span id="el_emprestimo_livro2_DATA_INICIAL_EMPRESTIMO_LIVRO">
<input type="text" data-table="emprestimo_livro2" data-field="x_DATA_INICIAL_EMPRESTIMO_LIVRO" name="x_DATA_INICIAL_EMPRESTIMO_LIVRO" id="x_DATA_INICIAL_EMPRESTIMO_LIVRO" size="30" maxlength="15" placeholder="<?php echo ew_HtmlEncode($emprestimo_livro2->DATA_INICIAL_EMPRESTIMO_LIVRO->getPlaceHolder()) ?>" value="<?php echo $emprestimo_livro2->DATA_INICIAL_EMPRESTIMO_LIVRO->EditValue ?>"<?php echo $emprestimo_livro2->DATA_INICIAL_EMPRESTIMO_LIVRO->EditAttributes() ?>>
</span>
<?php echo $emprestimo_livro2->DATA_INICIAL_EMPRESTIMO_LIVRO->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($emprestimo_livro2->DATA_FINAL_EMPRESTIMO_LIVRO->Visible) { // DATA_FINAL_EMPRESTIMO_LIVRO ?>
	<div id="r_DATA_FINAL_EMPRESTIMO_LIVRO" class="form-group">
		<label id="elh_emprestimo_livro2_DATA_FINAL_EMPRESTIMO_LIVRO" for="x_DATA_FINAL_EMPRESTIMO_LIVRO" class="<?php echo $emprestimo_livro2_add->LeftColumnClass ?>"><?php echo $emprestimo_livro2->DATA_FINAL_EMPRESTIMO_LIVRO->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $emprestimo_livro2_add->RightColumnClass ?>"><div<?php echo $emprestimo_livro2->DATA_FINAL_EMPRESTIMO_LIVRO->CellAttributes() ?>>
<span id="el_emprestimo_livro2_DATA_FINAL_EMPRESTIMO_LIVRO">
<input type="text" data-table="emprestimo_livro2" data-field="x_DATA_FINAL_EMPRESTIMO_LIVRO" name="x_DATA_FINAL_EMPRESTIMO_LIVRO" id="x_DATA_FINAL_EMPRESTIMO_LIVRO" size="30" maxlength="15" placeholder="<?php echo ew_HtmlEncode($emprestimo_livro2->DATA_FINAL_EMPRESTIMO_LIVRO->getPlaceHolder()) ?>" value="<?php echo $emprestimo_livro2->DATA_FINAL_EMPRESTIMO_LIVRO->EditValue ?>"<?php echo $emprestimo_livro2->DATA_FINAL_EMPRESTIMO_LIVRO->EditAttributes() ?>>
</span>
<?php echo $emprestimo_livro2->DATA_FINAL_EMPRESTIMO_LIVRO->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($emprestimo_livro2->STATUS_EMPRESTIMO->Visible) { // STATUS_EMPRESTIMO ?>
	<div id="r_STATUS_EMPRESTIMO" class="form-group">
		<label id="elh_emprestimo_livro2_STATUS_EMPRESTIMO" for="x_STATUS_EMPRESTIMO" class="<?php echo $emprestimo_livro2_add->LeftColumnClass ?>"><?php echo $emprestimo_livro2->STATUS_EMPRESTIMO->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $emprestimo_livro2_add->RightColumnClass ?>"><div<?php echo $emprestimo_livro2->STATUS_EMPRESTIMO->CellAttributes() ?>>
<span id="el_emprestimo_livro2_STATUS_EMPRESTIMO">
<input type="text" data-table="emprestimo_livro2" data-field="x_STATUS_EMPRESTIMO" name="x_STATUS_EMPRESTIMO" id="x_STATUS_EMPRESTIMO" size="30" placeholder="<?php echo ew_HtmlEncode($emprestimo_livro2->STATUS_EMPRESTIMO->getPlaceHolder()) ?>" value="<?php echo $emprestimo_livro2->STATUS_EMPRESTIMO->EditValue ?>"<?php echo $emprestimo_livro2->STATUS_EMPRESTIMO->EditAttributes() ?>>
</span>
<?php echo $emprestimo_livro2->STATUS_EMPRESTIMO->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div><!-- /page* -->
<?php if (!$emprestimo_livro2_add->IsModal) { ?>
<div class="form-group"><!-- buttons .form-group -->
	<div class="<?php echo $emprestimo_livro2_add->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("AddBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $emprestimo_livro2_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
</div><!-- /buttons .form-group -->
<?php } ?>
</form>
<script type="text/javascript">
femprestimo_livro2add.Init();
</script>
<?php
$emprestimo_livro2_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$emprestimo_livro2_add->Page_Terminate();
?>
