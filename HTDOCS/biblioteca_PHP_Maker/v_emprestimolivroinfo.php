...
$v_emprestimolivro = NULL;

//
// Table class for v_emprestimolivro
//
class cv_emprestimolivro extends cTable {
	...
	function __construct() {
		...
		$this->TableVar = 'v_emprestimolivro';
		$this->TableName = 'v_emprestimolivro';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "`v_emprestimolivro`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// ID_EMPRESTIMO_LIVRO
		$this->ID_EMPRESTIMO_LIVRO = new cField('v_emprestimolivro', 'v_emprestimolivro', 'x_ID_EMPRESTIMO_LIVRO', 'ID_EMPRESTIMO_LIVRO', '`ID_EMPRESTIMO_LIVRO`', '`ID_EMPRESTIMO_LIVRO`', 3, -1, FALSE, '`ID_EMPRESTIMO_LIVRO`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// NOME_ALUNO
		$this->NOME_ALUNO = new cField('v_emprestimolivro', 'v_emprestimolivro', 'x_NOME_ALUNO', 'NOME_ALUNO', '`NOME_ALUNO`', '`NOME_ALUNO`', 200, -1, FALSE, '`NOME_ALUNO`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->NOME_ALUNO->Sortable = TRUE; // Allow sort
		$this->fields['NOME_ALUNO'] = &$this->NOME_ALUNO;

		// NOME_LIVRO
		$this->NOME_LIVRO = new cField('v_emprestimolivro', 'v_emprestimolivro', 'x_NOME_LIVRO', 'NOME_LIVRO', '`NOME_LIVRO`', '`NOME_LIVRO`', 200, -1, FALSE, '`NOME_LIVRO`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->NOME_LIVRO->Sortable = TRUE; // Allow sort
		$this->fields['NOME_LIVRO'] = &$this->NOME_LIVRO;

		// NOME_CADASTRO
		$this->NOME_CADASTRO = new cField('v_emprestimolivro', 'v_emprestimolivro', 'x_NOME_CADASTRO', 'NOME_CADASTRO', '`NOME_CADASTRO`', '`NOME_CADASTRO`', 200, -1, FALSE, '`NOME_CADASTRO`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->NOME_CADASTRO->Sortable = TRUE; // Allow sort
		$this->fields['NOME_CADASTRO'] = &$this->NOME_CADASTRO;

		// DATA_INICIAL_EMPRESTIMO_LIVRO
		$this->DATA_INICIAL_EMPRESTIMO_LIVRO = new cField('v_emprestimolivro', 'v_emprestimolivro', 'x_DATA_INICIAL_EMPRESTIMO_LIVRO', 'DATA_INICIAL_EMPRESTIMO_LIVRO', '`DATA_INICIAL_EMPRESTIMO_LIVRO`', '`DATA_INICIAL_EMPRESTIMO_LIVRO`', 200, -1, FALSE, '`DATA_INICIAL_EMPRESTIMO_LIVRO`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		$this->DATA_FINAL_EMPRESTIMO_LIVRO = new cField('v_emprestimolivro', 'v_emprestimolivro', 'x_DATA_FINAL_EMPRESTIMO_LIVRO', 'DATA_FINAL_EMPRESTIMO_LIVRO', '`DATA_FINAL_EMPRESTIMO_LIVRO`', '`DATA_FINAL_EMPRESTIMO_LIVRO`', 200, -1, FALSE, '`DATA_FINAL_EMPRESTIMO_LIVRO`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// STATUS_EMPRESTIMO
		$this->STATUS_EMPRESTIMO = new cField('v_emprestimolivro', 'v_emprestimolivro', 'x_STATUS_EMPRESTIMO', 'STATUS_EMPRESTIMO', '`STATUS_EMPRESTIMO`', '`STATUS_EMPRESTIMO`', 3, -1, FALSE, '`STATUS_EMPRESTIMO`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`v_emprestimolivro`";
	}
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$this->ID_EMPRESTIMO_LIVRO->setDbValue($conn->Insert_ID());
			$rs['ID_EMPRESTIMO_LIVRO'] = $this->ID_EMPRESTIMO_LIVRO->DbValue;
		}
		return $bInsert;
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('ID_EMPRESTIMO_LIVRO', $rs))
				ew_AddFilter($where, ew_QuotedName('ID_EMPRESTIMO_LIVRO', $this->DBID) . '=' . ew_QuotedValue($rs['ID_EMPRESTIMO_LIVRO'], $this->ID_EMPRESTIMO_LIVRO->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`ID_EMPRESTIMO_LIVRO` = @ID_EMPRESTIMO_LIVRO@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->ID_EMPRESTIMO_LIVRO->CurrentValue))
			return "0=1"; // Invalid key
		if (is_null($this->ID_EMPRESTIMO_LIVRO->CurrentValue))
			return "0=1"; // Invalid key
		else
			$sKeyFilter = str_replace("@ID_EMPRESTIMO_LIVRO@", ew_AdjustSql($this->ID_EMPRESTIMO_LIVRO->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "v_emprestimolivrolist.php";
		}
	}
	...
	function GetModalCaption($pageName) {
		global $Language;
		if ($pageName == "v_emprestimolivroview.php")
			return $Language->Phrase("View");
		elseif ($pageName == "v_emprestimolivroedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "v_emprestimolivroadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("v_emprestimolivroview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("v_emprestimolivroview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "v_emprestimolivroadd.php?" . $this->UrlParm($parm);
		else
			$url = "v_emprestimolivroadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("v_emprestimolivroedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("v_emprestimolivroadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("v_emprestimolivrodelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "ID_EMPRESTIMO_LIVRO:" . ew_VarToJson($this->ID_EMPRESTIMO_LIVRO->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->ID_EMPRESTIMO_LIVRO->CurrentValue)) {
			$sUrl .= "ID_EMPRESTIMO_LIVRO=" . urlencode($this->ID_EMPRESTIMO_LIVRO->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$isPost = ew_IsPost();
			if ($isPost && isset($_POST["ID_EMPRESTIMO_LIVRO"]))
				$arKeys[] = $_POST["ID_EMPRESTIMO_LIVRO"];
			elseif (isset($_GET["ID_EMPRESTIMO_LIVRO"]))
				$arKeys[] = $_GET["ID_EMPRESTIMO_LIVRO"];
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->ID_EMPRESTIMO_LIVRO->CurrentValue = $key;
