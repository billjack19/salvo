...
$cadastro_view = NULL; // Initialize page object first

class ccadastro_view extends ccadastro {
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["ID_CADASTRO"] <> "") {
			$this->RecKey["ID_CADASTRO"] = $_GET["ID_CADASTRO"];
			$KeyUrl .= "&amp;ID_CADASTRO=" . urlencode($this->RecKey["ID_CADASTRO"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["ID_CADASTRO"] <> "") {
				...
				$this->RecKey["ID_CADASTRO"] = $this->ID_CADASTRO->QueryStringValue;
			} elseif (@$_POST["ID_CADASTRO"] <> "") {
				$this->ID_CADASTRO->setFormValue($_POST["ID_CADASTRO"]);
				$this->RecKey["ID_CADASTRO"] = $this->ID_CADASTRO->FormValue;
			} else {
				$sReturnUrl = "cadastrolist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "cadastrolist.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "cadastrolist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($cadastro_view)) $cadastro_view = new ccadastro_view();

// Page init
$cadastro_view->Page_Init();

// Page main
$cadastro_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$cadastro_view->Page_Render();
?>
...
var CurrentForm = fcadastroview = new ew_Form("fcadastroview", "view");

// Form_CustomValidate event
fcadastroview.Form_CustomValidate = 
...
fcadastroview.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
<div class="ewToolbar">
<?php $cadastro_view->ExportOptions->Render("body") ?>
<?php
	foreach ($cadastro_view->OtherOptions as &$option)
		$option->Render("body");
?>
<div class="clearfix"></div>
</div>
<?php $cadastro_view->ShowPageHeader(); ?>
<?php
$cadastro_view->ShowMessage();
?>
<form name="fcadastroview" id="fcadastroview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($cadastro_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $cadastro_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="cadastro">
<input type="hidden" name="modal" value="<?php echo intval($cadastro_view->IsModal) ?>">
<table class="table table-striped table-bordered table-hover table-condensed ewViewTable">
<?php if ($cadastro->ID_CADASTRO->Visible) { // ID_CADASTRO ?>
	<tr id="r_ID_CADASTRO">
		<td class="col-sm-2"><span id="elh_cadastro_ID_CADASTRO"><?php echo $cadastro->ID_CADASTRO->FldCaption() ?></span></td>
...
<?php echo $cadastro->ID_CADASTRO->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($cadastro->NOME_CADASTRO->Visible) { // NOME_CADASTRO ?>
	<tr id="r_NOME_CADASTRO">
		<td class="col-sm-2"><span id="elh_cadastro_NOME_CADASTRO"><?php echo $cadastro->NOME_CADASTRO->FldCaption() ?></span></td>
...
<?php echo $cadastro->NOME_CADASTRO->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($cadastro->LOGIN_CADASTRO->Visible) { // LOGIN_CADASTRO ?>
	<tr id="r_LOGIN_CADASTRO">
		<td class="col-sm-2"><span id="elh_cadastro_LOGIN_CADASTRO"><?php echo $cadastro->LOGIN_CADASTRO->FldCaption() ?></span></td>
...
<?php echo $cadastro->LOGIN_CADASTRO->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($cadastro->SENHA_CADASTRO->Visible) { // SENHA_CADASTRO ?>
	<tr id="r_SENHA_CADASTRO">
		<td class="col-sm-2"><span id="elh_cadastro_SENHA_CADASTRO"><?php echo $cadastro->SENHA_CADASTRO->FldCaption() ?></span></td>
...
<?php echo $cadastro->SENHA_CADASTRO->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($cadastro->ID_NIVEL_ACESSO->Visible) { // ID_NIVEL_ACESSO ?>
	<tr id="r_ID_NIVEL_ACESSO">
		<td class="col-sm-2"><span id="elh_cadastro_ID_NIVEL_ACESSO"><?php echo $cadastro->ID_NIVEL_ACESSO->FldCaption() ?></span></td>
...
<?php echo $cadastro->ID_NIVEL_ACESSO->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
