...
$nivel_acesso_delete = NULL; // Initialize page object first

class cnivel_acesso_delete extends cnivel_acesso {
	...
	// Page object name
	var $PageObjName = 'nivel_acesso_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("nivel_acessolist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in nivel_acesso class, nivel_acessoinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("nivel_acessolist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($nivel_acesso_delete)) $nivel_acesso_delete = new cnivel_acesso_delete();

// Page init
$nivel_acesso_delete->Page_Init();

// Page main
$nivel_acesso_delete->Page_Main();
...
var CurrentForm = fnivel_acessodelete = new ew_Form("fnivel_acessodelete", "delete");

// Form_CustomValidate event
fnivel_acessodelete.Form_CustomValidate = 
...
fnivel_acessodelete.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
?>
<form name="fnivel_acessodelete" id="fnivel_acessodelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($nivel_acesso_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $nivel_acesso_delete->Token ?>">
<?php } ?>
...
<?php if ($nivel_acesso->ID_NIVEL_ACESSO->Visible) { // ID_NIVEL_ACESSO ?>
		<th class="<?php echo $nivel_acesso->ID_NIVEL_ACESSO->HeaderCellClass() ?>"><span id="elh_nivel_acesso_ID_NIVEL_ACESSO" class="nivel_acesso_ID_NIVEL_ACESSO"><?php echo $nivel_acesso->ID_NIVEL_ACESSO->FldCaption() ?></span></th>
<?php } ?>
<?php if ($nivel_acesso->DESCRICAO_NIVEL_ACESSO->Visible) { // DESCRICAO_NIVEL_ACESSO ?>
		<th class="<?php echo $nivel_acesso->DESCRICAO_NIVEL_ACESSO->HeaderCellClass() ?>"><span id="elh_nivel_acesso_DESCRICAO_NIVEL_ACESSO" class="nivel_acesso_DESCRICAO_NIVEL_ACESSO"><?php echo $nivel_acesso->DESCRICAO_NIVEL_ACESSO->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
...
while (!$nivel_acesso_delete->Recordset->EOF) {
	...
	$nivel_acesso->RowType = EW_ROWTYPE_VIEW; // View
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $nivel_acesso_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
fnivel_acessodelete.Init();
</script>
<?php
...
<?php
$nivel_acesso_delete->Page_Terminate();
?>
