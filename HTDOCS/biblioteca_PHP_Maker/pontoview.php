...
$ponto_view = NULL; // Initialize page object first

class cponto_view extends cponto {
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["ID_PONTO"] <> "") {
			$this->RecKey["ID_PONTO"] = $_GET["ID_PONTO"];
			$KeyUrl .= "&amp;ID_PONTO=" . urlencode($this->RecKey["ID_PONTO"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["ID_PONTO"] <> "") {
				...
				$this->RecKey["ID_PONTO"] = $this->ID_PONTO->QueryStringValue;
			} elseif (@$_POST["ID_PONTO"] <> "") {
				$this->ID_PONTO->setFormValue($_POST["ID_PONTO"]);
				$this->RecKey["ID_PONTO"] = $this->ID_PONTO->FormValue;
			} else {
				$sReturnUrl = "pontolist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "pontolist.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "pontolist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($ponto_view)) $ponto_view = new cponto_view();

// Page init
$ponto_view->Page_Init();

// Page main
$ponto_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$ponto_view->Page_Render();
?>
...
var CurrentForm = fpontoview = new ew_Form("fpontoview", "view");

// Form_CustomValidate event
fpontoview.Form_CustomValidate = 
...
fpontoview.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
<div class="ewToolbar">
<?php $ponto_view->ExportOptions->Render("body") ?>
<?php
	foreach ($ponto_view->OtherOptions as &$option)
		$option->Render("body");
?>
<div class="clearfix"></div>
</div>
<?php $ponto_view->ShowPageHeader(); ?>
<?php
$ponto_view->ShowMessage();
?>
<form name="fpontoview" id="fpontoview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($ponto_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $ponto_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="ponto">
<input type="hidden" name="modal" value="<?php echo intval($ponto_view->IsModal) ?>">
<table class="table table-striped table-bordered table-hover table-condensed ewViewTable">
<?php if ($ponto->ID_PONTO->Visible) { // ID_PONTO ?>
	<tr id="r_ID_PONTO">
		<td class="col-sm-2"><span id="elh_ponto_ID_PONTO"><?php echo $ponto->ID_PONTO->FldCaption() ?></span></td>
...
<?php echo $ponto->ID_PONTO->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($ponto->ID_FUNCIONARIO->Visible) { // ID_FUNCIONARIO ?>
	<tr id="r_ID_FUNCIONARIO">
		<td class="col-sm-2"><span id="elh_ponto_ID_FUNCIONARIO"><?php echo $ponto->ID_FUNCIONARIO->FldCaption() ?></span></td>
...
<?php echo $ponto->ID_FUNCIONARIO->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($ponto->DATA_PONTO->Visible) { // DATA_PONTO ?>
	<tr id="r_DATA_PONTO">
		<td class="col-sm-2"><span id="elh_ponto_DATA_PONTO"><?php echo $ponto->DATA_PONTO->FldCaption() ?></span></td>
...
<?php echo $ponto->DATA_PONTO->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($ponto->HORARIO_INICIAL_PONTO->Visible) { // HORARIO_INICIAL_PONTO ?>
	<tr id="r_HORARIO_INICIAL_PONTO">
		<td class="col-sm-2"><span id="elh_ponto_HORARIO_INICIAL_PONTO"><?php echo $ponto->HORARIO_INICIAL_PONTO->FldCaption() ?></span></td>
...
<?php echo $ponto->HORARIO_INICIAL_PONTO->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($ponto->HORARIO_FINAL_PONTO->Visible) { // HORARIO_FINAL_PONTO ?>
	<tr id="r_HORARIO_FINAL_PONTO">
		<td class="col-sm-2"><span id="elh_ponto_HORARIO_FINAL_PONTO"><?php echo $ponto->HORARIO_FINAL_PONTO->FldCaption() ?></span></td>
...
<?php echo $ponto->HORARIO_FINAL_PONTO->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
