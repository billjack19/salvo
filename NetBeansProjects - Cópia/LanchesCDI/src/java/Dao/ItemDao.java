...
public class ItemDao {
    ...
    //private String[] arrayHoraComplemento;
    //private String horaImpressao;
    private String complemetoItem;
    
    private String adicionalItem;
    private int contAdicional;
    private String preparoItem;
    private int contPreparo;
    private String excessaoItem;
    private int contExcassao;
    ...
    public List<Item> listarItensPreparo(int item) {
        ...
        try {
            sql = "SELECT tapI.DescricaoPreparo, tapI.Sequencia"
                    ...
                    + " WHERE tapI.codigo = "
                    + "("
                    + "SELECT Preparo FROM ITEM WHERE ITEM = " + item
                    + " )"
    ...
    public List<Item> listarItensAdicional(int item, int filial) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                _Item.setSubGrupoItem(rs.getInt("SUB_GRUPO"));
                _Item.setDescricao(
                        rs.getString("DESCRICAO") + "++" + Integer.toString(rs.getInt("QTD_NAO_COBRAR"))
                );
                _Item.setUnidade_medida(
    ...
    public List<Item> listarTodosItens(int filial) {
        List<Item> Items = new ArrayList<>();
        Item _Item = null;
        try {
            sql = "SELECT itf.PRECO_BASE, i.ITEM, i.GRUPO, i.DESCRICAO, i.UNIDADE_MEDIDA, i.SUB_GRUPO"
                    ...
                    //+ " WHERE i.SUB_GRUPO = " + subGrupoItem
                    //+ " AND i.GRUPO = " + grupo
                    + " WHERE itf.filial = " + filial
                    //+ " AND itf.PRECO_BASE > 0"
                    + " ORDER BY DESCRICAO;";
    ...
    public List<Item> itemCodigo (int codigo, int filial){
        List<Item> Items = new ArrayList<>();
        Item _Item = null;
        try {
            sql = "SELECT itf.PRECO_BASE, i.ITEM, i.GRUPO, i.DESCRICAO, i.UNIDADE_MEDIDA, i.SUB_GRUPO"
    ...
    public String listarLancPedidosItensAdicional(int lancPedidoItem) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                if (cont == 0) {
                    resultado = /*Integer.toString(rs.getInt("item")) +*/ rs.getString("DESCRICAO") + seguencia;
                } else {
                    resultado = resultado + ", " + /*Integer.toString(rs.getInt("item")) +*/ rs.getString("DESCRICAO") + seguencia;
                }
                cont++;
            }
            Conexao.fecharConexao(con, pst, rs);
    ...
    public String listarLancPedidosItensAdicionalSPreco(int lancPedidoItem) {
        String resultado = null;
        try {
            cont = 0;
            sql = "SELECT i.DESCRICAO" // , iex.VALOR_UNITARIO
            ...
            while (rs.next()) {
                ...
                if (cont == 0) {
                    resultado = /*Integer.toString(rs.getInt("item")) +*/ rs.getString("DESCRICAO");// + " (R$ " + seguencia + ")";
                } else {
                    resultado = resultado + ", " + /*Integer.toString(rs.getInt("item")) +*/ rs.getString("DESCRICAO");// + " (R$ " + seguencia + ")";
                }
                cont++;
            }
            Conexao.fecharConexao(con, pst, rs);
    ...
    public int adicionarItem(LancPedidosItens _LancPedidosItens) {
        try {
            if (atualizarValorPedido(_LancPedidosItens.getFilial(), _LancPedidosItens.getDocumento(), _LancPedidosItens.getValorTotal(), "+")) {
                ...
                pst.setString(13, Float.toString(_LancPedidosItens.getQuantidade()));
                pst.execute();
    ...
    public boolean adicionarItemImpressao(LancPedidosItens _LancPedidosItens) {
        try {
            complemetoItem = this.complementoImpressao(_LancPedidosItens.getIdLancPedido(), _LancPedidosItens.getItem());
            ...
            pst.setString(4, Integer.toString(_LancPedidosItens.getItem()));
            pst.setString(5, Float.toString(_LancPedidosItens.getQuantidade()));
            pst.setString(6, complemetoItem);
    ...
    public String complementoImpressao(int idLancPedidoItem, int item) {
        try {
            sql = ""
                    // TABELA ADICIONAL
                    + "SELECT "
                    ...
                    + " VALOR_UNITARIO "
                    + " FROM "
                    + " Lanc_Pedidos_Itens_Adicional "
                    + " INNER JOIN ITEM ON ITEM.ITEM = Lanc_Pedidos_Itens_Adicional.ITEM "
                    + " WHERE Lanc_Pedidos_Itens_Adicional.ID_TabAdicionalProdutosItens = " + idLancPedidoItem
                    + " UNION ALL "
                    // TABELA EXCESSAO
                    + " SELECT "
                    + " 'E' AS TIPO, "
                    ...
                    + " FROM Lanc_pedidos_itens_Excessao "
                    + " INNER JOIN ITEM ON ITEM.ITEM = Lanc_pedidos_itens_Excessao.ITEM "
                    + " WHERE Lanc_pedidos_itens_Excessao.ID_Lanc_Pedidos_itens = " + idLancPedidoItem
                    + " UNION ALL"
                    // TABELA PREPARO
                    + " SELECT "
                    + " 'P' AS TIPO, "
                    + " DescricaoPreparo AS DESCRICAO, "
                    + " NULL AS VALOR_UNITARIO "
                    + " FROM Lanc_Pedidos_Itens_Preparo "
                    + " WHERE ID_TabPreparoProdutosItens = " + idLancPedidoItem;
            con = Conexao.conexao();
            pst = con.prepareStatement(sql);
            rs = pst.executeQuery();
            ...
            while (rs.next()) {
                if (null != rs.getString("TIPO")) {
                    switch (rs.getString("TIPO")) {
                        case "P":
                            if (contPreparo == 0) {
                                preparoItem += "<br>Preparo: " + rs.getString("DESCRICAO");
                            } else {
                                preparoItem += ", " + rs.getString("DESCRICAO");
                            }
                            contPreparo++;
                            break;
                        case "A":
                            if (contAdicional == 0) {
                                adicionalItem += "<br>Adicional: " + rs.getString("DESCRICAO"); // + " (R$ " + rs.getString("VALOR_UNITARIO") + ") "
                            } else {
                                adicionalItem += ", " + rs.getString("DESCRICAO"); // + " (R$ " + rs.getString("VALOR_UNITARIO") + ")"
                            }
                            contAdicional++;
                            break;
                        case "E":
                            if (contExcassao == 0) {
                                excessaoItem += "<br>Excessão: " + rs.getString("DESCRICAO");
                            } else {
                                excessaoItem += ", " + rs.getString("DESCRICAO");
                            }
                            contExcassao++;
                            break;
                        default:
            ...
            sql = "SELECT DESCRICAO FROM ITEM WHERE item = " + item;
            con = Conexao.conexao();
            pst = con.prepareStatement(sql);
            rs = pst.executeQuery();
            while (rs.next()) {
                complemetoItem = rs.getString("DESCRICAO") + excessaoItem + preparoItem + adicionalItem;
            }
            Conexao.fecharConexao(con, pst, rs);
            
            return complemetoItem;
        } catch (SQLException ex) {
            //Logger.getLogger(ItemDao.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public boolean setarAdicionalProduto(int idLancPedidoItem){
        try{
            sql = ""
                    // TABELA ADICIONAL
                    + "SELECT "
            ...
            contExcassao = 0;
            while (rs.next()) {
                if(null != rs.getString("TIPO")) switch (rs.getString("TIPO")) {
                    case "P":
                        if(contPreparo == 0) preparoItem += " \nPreparo: "+rs.getString("DESCRICAO");
                        else  preparoItem += ", \n"+rs.getString("DESCRICAO");
                        contPreparo++;
                        break;
                    case "A":
                        if(contAdicional == 0) adicionalItem += " \nAdicional: "+rs.getString("DESCRICAO") + " (R$ " + rs.getString("VALOR_UNITARIO") + ") ";
                        else adicionalItem += ", \n"+rs.getString("DESCRICAO") + " (R$ " + rs.getString("VALOR_UNITARIO") + ")";
                        contAdicional++;
                        break;
                    case "E":
                        if(contExcassao == 0) excessaoItem += "Excessão: "+rs.getString("DESCRICAO");
                        else excessaoItem += ", \n"+rs.getString("DESCRICAO");
                        contExcassao++;
                        break;
                    default:
            ...
            Conexao.fecharConexao(con, pst, rs);
            complemetoItem = excessaoItem + preparoItem + adicionalItem;
            
            sql = "UPDATE Lanc_Pedidos_itens"
                    + " SET AdicionalProduto = '" + complemetoItem + "'"
                    + " WHERE ID_Lanc_Pedidos_Itens = " + idLancPedidoItem;
    ...
    public String removerItem(int filial, String documento, int sequencia) {
        try {
